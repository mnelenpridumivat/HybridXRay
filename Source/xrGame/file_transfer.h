...
namespace file_transfer
{
    ...
    public:
        ...
        void               start_transfer_file(shared_str const& file_name, ClientID const& to_client, ClientID const& from_client, sending_state_callback_t& tstate_callback);
        // this method used by proxies, to transmit file by portions through the memory writer ...
        void               start_transfer_file(CMemoryWriter& mem_writer, u32 mem_writer_max_size, ClientID const& to_client, ClientID const& from_client, sending_state_callback_t& tstate_callback, u32 const user_param);
        void               start_transfer_file(u8* data_ptr, u32 const data_size, ClientID const& to_client, ClientID const& from_client, sending_state_callback_t& tstate_callback, u32 const user_param);
        void               start_transfer_file(buffer_vector<mutable_buffer_t>& vector_of_buffers, ClientID const& to_client, ClientID const& from_client, sending_state_callback_t& tstate_callback, u32 const user_param);

        void               stop_transfer_file(dst_src_pair_t const& tofrom);

        filereceiver_node* start_receive_file(shared_str const& file_name, ClientID const& from_client, receiving_state_callback_t& rstate_callback);
        filereceiver_node* start_receive_file(CMemoryWriter& mem_writer, ClientID const& from_client, receiving_state_callback_t& rstate_callback);
