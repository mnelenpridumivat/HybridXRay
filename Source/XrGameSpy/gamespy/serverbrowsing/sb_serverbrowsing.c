...
#ifdef GSI_UNICODE
ServerBrowser ServerBrowserNewW(
    const unsigned short* queryForGamename,
    ...
    void*                 instance)
{
    ...
    return ServerBrowserNewA(
        forGameName_A, fromGameName_A, fromGameKey_A, queryFromVersion, maxConcUpdates, queryVersion, lanBrowse,
        callback, instance);
}
#endif
...
static SBError WaitForTriggerUpdate(ServerBrowser sb, SBBool viaMaster)
{
    ...
    while (sb->triggerIP != 0 && err == sbe_noerror)
    {
        ...
        err = ServerBrowserThink(sb);
        if (viaMaster &&
            sb->list.state == sb_disconnected)   // we were supposed to get from master, and it's disconnected
            break;
    }
    return err;
}
