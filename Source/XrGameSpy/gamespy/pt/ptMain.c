    ...
    char**                 mirrorURLs)
{
    ...
    if (data->callback)
    {
        ...
        else
        {
#ifndef GSI_UNICODE
            data->callback(
                data->fileID, found, description, size, numMirrors, (const char**)mirrorNames, (const char**)mirrorURLs,
                data->param);
#else
            ...
            data->callback(
                data->fileID, found, description_W, size_W, numMirrors, (const unsigned short**)mirrorNames_W,
                (const unsigned short**)mirrorURLs_W, data->param);
    ...
    void*           param)
{
    ...
    charsWritten = snprintf(
        aURL, PTA_MAX_STRING_SIZE, "%s?productid=%d&versionuniqueid=%s&distid=%d&gamename=%s", gPTAVercheckURL,
        productID, versionUniqueID, distributionID, __GSIACGamename);
...
PTBool ptTrackUsageA(int userID, int productID, const char* versionUniqueID, int distributionID, PTBool blocking)
{
    ...
    /////////////////
    charsWritten = snprintf(
        aURL, PTA_MAX_STRING_SIZE, "%s?userid=%d&productid=%d&versionuniqueid=%s&distid=%d&uniqueid=%s&gamename=%s",
        gPTAMOTDURL, userID, productID, versionUniqueID, distributionID, GOAGetUniqueID(), __GSIACGamename);
    assert(charsWritten >= 0);
    if (charsWritten < 0)
        return PTFalse;
    // Send the info.
    ...
    void*           param)
{
    ...
    charsWritten = snprintf(
        aURL, PTA_MAX_STRING_SIZE, "%s?userid=%d&productid=%d&versionuniqueid=%s&distid=%d&uniqueid=%s&gamename=%s",
        gPTAVercheckURL, userID, productID, versionUniqueID, distributionID, GOAGetUniqueID(), __GSIACGamename);
