...
#define GT2True  1
    ...
    *********************/
    ...
    GT2Result    gt2CreateSocket(GT2Socket* socket,               // if the result is GT2Success, the socket object handle will be stored at this address
           const char*                      localAddress,         // the local address to bind to
           int                              outgoingBufferSize,   // size of per-connection buffer where sent messages waiting to be confirmed are held,
                                                                  // use 0 for default
           int                              incomingBufferSize,   // size of per-connection buffer where out-of-order received messages are held, use 0
                                                                  // for default
    ...
    GT2Result    gt2CreateAdHocSocket(GT2Socket* socket,               // if the result is GT2Success, the socket object handle will be stored at this address
           const char*                           localAddress,         // the local address to bind to
           int                                   outgoingBufferSize,   // size of per-connection buffer where sent messages waiting to be confirmed are held,
                                                                       // use 0 for default
           int                                   incomingBufferSize,   // size of per-connection buffer where out-of-order received messages are held, use 0
    ...
    // and it needs to hold any messages that arrive out of order. if either buffer runs out
    // of space the connection will be dropped.
    GT2Result gt2CreateVDPSocket(GT2Socket* socket,               // if the result is GT2Success, the socket object handle will be stored at this address
        const char*                         localAddress,         // the local address to bind to
        int                                 outgoingBufferSize,   // size of per-connection buffer where sent messages waiting to be confirmed are held,
                                                                  // use 0 for default
        int                                 incomingBufferSize,   // size of per-connection buffer where out-of-order received messages are held, use 0
    ...
    *************************/
    ...
    GT2Result          gt2Connect(GT2Socket socket,          // the local socket to use for the connection
                 GT2Connection*             connection,      // if the result is GT2Success, and blocking is false, the connection  object
    ...
    *******************/
    ...
    // GT2Closing - the connection is in the process of closing (i.e., sent a close message and waiting for
    // confirmation). GT2Closed - the connection has already been closed and will soon be freed
    ...
    **********************/
    ...
    // of the functionality.  They are provided so that code that only needs certain information can be a little
    // simpler. Before using these, read the comments for the gt2*ToHostInfo() functions, as the info also applies to
    // these functions.
    ...
    *******************/
    ...
    // sets a callback to be called whenever a UDP datagram is sent or received, and when a connection reset is
    // received. pass in a callback of NULL to remove the callback.  the dumps sit at a lower level than the filters,
    // and allow an app to keep an eye on exactly what datagrams are being sent and received, allowing for close
    // monitoring.  however the dumps cannot be used to modify data, only monitor it.  the dumps are useful for
    // debugging purposes, and to keep track of data send and receive rates (e.g., the Quake 3 engine's netgraph). note
    // that these are the actual UDP datagrams being sent and received - datagrams may be dropped, repeated, or
