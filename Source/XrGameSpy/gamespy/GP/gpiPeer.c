...
static GPResult gpiProcessPeerInitiatingConnection(GPConnection* connection, GPIPeer* peer)
{
    ...
    switch (peer->state)
    {
        ...
        case GPI_PEER_GOT_SIG: {
            ...
            /////////////////////
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_State, GSIDebugLevel_Verbose, "Got the peer signature for profileid: %d\n",
                peer->profile);
    ...
    for (peer = iconnection->peerList; peer != NULL; peer = nextPeer)
    {
        ...
        else
        {
            ...
            if ((peer->state == GPI_PEER_DISCONNECTED) || (result != GP_NO_ERROR) || (time(NULL) > peer->timeout))
            {
                ...
                /////////////
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Notice, "Peer disconnected, pid: %d",
                    peer->profile);
                gpiRemovePeer(connection, peer);
            }
        }
...
// No sockets after the first will be able to bind
    ...
    */
    ...
    if (profile->buddyStatusInfo)
    {
        ...
        {
            anError = gsUdpEngineStartTalkingToPeer(
                profile->buddyStatusInfo->buddyIp, profile->buddyStatusInfo->buddyPort, iconnection->mHeader,
                GPI_PEER_TIMEOUT);
...
void gpiPeerLeftCallback(unsigned int ip, unsigned short port, GSUdpCloseReason reason, void* userData)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Notice, "Peer left: addr: %s:%d, profile: %d\n",
    ...
    void*          userData)
{
    ...
    if (!aPeer)
    {
        ...
        {
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Memory, GSIDebugLevel_HotError,
    ...
    void*          userData)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_HotError, "Peer does not exist: ip-port: %s:%d\n",
    ...
    else
    {
        if (rejected)
        {
            aPeer->state = GPI_PEER_DISCONNECTED;
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Notice,
    ...
    gsDebugFormat(
        GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Notice, "Peer Connection accepted: ip-port: %s:%d\n",
        inet_ntoa(anAddr), port);
    ...
    else
    {
        ...
        for (aPrevOp = peer->peerOpQueue.first; aPrevOp->next != operation; aPrevOp = aPrevOp->next)
        {
            if (aPrevOp->next == NULL)
            {
                ...
                ////////////////////////////////////
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
