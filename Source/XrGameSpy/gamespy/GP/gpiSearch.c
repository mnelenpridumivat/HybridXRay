...
GPResult gpiProfileSearchUniquenick(
    GPConnection* connection,
    const char    uniquenick[GP_UNIQUENICK_LEN],
    ...
    do
    {
        ...
        if (operation->state == GPI_CONNECTING)
        {
            ...
            if (state == GPI_DISCONNECTED)
                CallbackError(
                    connection, GP_SERVER_ERROR, GP_SEARCH_CONNECTION_FAILED,
        ...
        else if (operation->state == GPI_WAITING)
        {
            ...
            if (result != GP_NO_ERROR)
            {
                if (result == GP_NETWORK_ERROR)
                    CallbackError(
                        connection, GP_NETWORK_ERROR, GP_SEARCH_CONNECTION_FAILED,
            ...
            if (strstr(data->inputBuffer.buffer, "\\final\\") != NULL)
            {
                ...
                if ((data->type == GPI_SEARCH_PROFILE) || (data->type == GPI_SEARCH_PROFILE_UNIQUENICK))
                {
                    ...
                    if ((more == GP_MORE) && (arg.more == GP_MORE))
                        CHECK_RESULT(gpiProfileSearch(
                            connection, data->nick, data->uniquenick, data->email, data->firstname, data->lastname,
                            data->icquin, arg.numMatches + data->skip, (GPEnum)operation->blocking,
                            operation->callback.callback, operation->callback.param));
