...
static GPResult gpiReadDiskProfile(GPConnection* connection, GPIBool* failedOut)
{
    ...
    gsDebugFormat(
        GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_Comment, "Reading profile %d from disk cache:\n",
        profile.profileId);
    ...
    do
    {
        ...
        if (!failed)
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_HotError, "Unrecognized profile key: %s=%s\n", key,
                    value);
            }
        }
    } while (!failed);
