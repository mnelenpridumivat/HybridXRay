    ...
    ********/
        ...
        void*                       userdata)
    {
        ...
        if (ispublic)
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "%s resolved to %s\r\n",
                    override ? qr2_hostname : hostname, inet_ntoa(cr->hbaddr.sin_addr));
            }
            else
            {
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_HotError, "Failed on DNS lookup for %s \r\n",
        ...
        {
            gsDebugFormat(
                GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_HotError,
        ...
        void*                       userdata)
    {
        ...
        return qr2_init_socketA(
            qrec, s, boundport, gamename_A, secretkey_A, ispublic, natnegotiate, server_key_callback,
    ...
    int saddrlen;
        ...
        if (baseport == 0)   // we bound it dynamically
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_HotError,
        ...
        *port = baseport;
        ...
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment,
        ...
        void*                       userdata)
    {
        ...
        ret = qr2_init_socketA(
            qrec, hbsock, baseport, gamename, secret_key, ispublic, natnegotiate, server_key_callback,
            player_key_callback, team_key_callback, key_list_callback, playerteam_count_callback, adderror_callback,
            userdata);
        if (qrec == NULL)
            qrec = &current_rec;
        ...
        void*                       userdata)
    {
        ...
        return qr2_initA(
            qrec, (ip != NULL) ? ip_A : NULL, baseport, gamename_A, secretkey_A, ispublic, natnegotiate,
    ...
    {
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_StackTrace, "qr2_register_natneg_callback()\r\n");
    ...
    {
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_StackTrace,
    ...
    void qr2_check_send_heartbeat(qr2_t qrec)
    {
        ...
        if (current_time() - qrec->lastka >
            KA_TIME)   // send a keep alive (to keep NAT port mappings the same if possible)
            send_keepalive(qrec);
    }
    ...
    void qr2_send_statechanged(qr2_t qrec)
    {
        ...
        {
            gsDebugFormat(
                GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_Warning,
        ...
        {
            gsDebugFormat(
                GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_Notice,
...
#define MAX_CHALLENGE 64
    ...
    static void handle_public_address(qr2_t qrec, char* buffer)
    {
...
#ifdef GSI_COMMON_DEBUG
        {
            ...
            addr.s_addr = ip;
            gsDebugFormat(
                GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice, "Received public address (%s:%d)\r\n",
                inet_ntoa(addr), port);
        }
#endif
    ...
    static void
        qr_build_partial_query_reply(qr2_t qrec, qr2_buffer_t buf, qr2_key_type keytype, int keycount, uchar* keys)
    {
    ...
    static void qr_build_query_reply(
        qr2_t        qrec,
        qr2_buffer_t buf,
        int          serverkeycount,
        uchar*       serverkeys,
    ...
    static void qr_process_query(qr2_t qrec, qr2_buffer_t buf, uchar* qdata, int len, struct sockaddr* sender)
    {
        ...
        {
            gsDebugFormat(
                GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_WarmError,
        ...
        else
        {
            ...
            qr_build_query_reply(
                qrec, buf, serverkeycount, serverkeys, playerkeycount, playerkeys, teamkeycount, teamkeys);
            sendto(qrec->hbsock, buf->buffer, buf->len, 0, sender, sizeof(struct sockaddr_in));
        }
    ...
    */
    ...
    {
        unsigned char natNegBytes[NATNEG_MAGIC_LEN] = {NN_MAGIC_0, NN_MAGIC_1, NN_MAGIC_2,
    ...
    static void qr2_expire_ip_verify(qr2_t qrec)
    {
        ...
        for (; i < QR2_IPVERIFY_ARRAY_SIZE; i++)
        {
            if (qrec->ipverify[i].addr.sin_addr.s_addr != 0 &&
                (now - qrec->ipverify[i].createtime > QR2_IPVERIFY_TIMEOUT))
                qrec->ipverify[i].addr.sin_addr.s_addr = 0;
        }
    }
    ...
    void qr2_parse_queryA(qr2_t qrec, char* query, int len, struct sockaddr* sender)
    {
        ...
        if (query[0] == 0x3B) /* a cdkey query */
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice,
...
// #define SIMULATE_HARD_FIREWALL
        ...
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Warning,
...
// #define SIMULATE_FIREWALL
        ...
        switch (ptype)
        {
            ...
            case PACKET_QUERY:
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice, "Processing query packet\r\n");
            ...
            case PACKET_CHALLENGE:
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice, "Processing challenge packet\r\n");
                ...
                if (len >= (PUBLIC_ADDR_LEN + 3))
                {
                    ...
                    gsDebugFormat(
                        GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_Notice, "Received setting options: %d\r\n",
                        qrec->backendoptions);
            ...
            case PACKET_ECHO:
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice, "Processing echo packet\r\n");
            ...
            case PACKET_ADDERROR:
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_WarmError, "Processing adderror packet\r\n");
            ...
            case PACKET_KEEPALIVE:
                gsDebugFormat(
                    GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "Processing keepalive packet\r\n");
                return;   // if we get a keep alive, ignore it and return (just used to tell us the server knows about
                          // us)
            default:
        ...
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "Sent %d bytes as QR2 query response\r\n",
            buf.len);
    ...
    static void send_keepalive(qr2_t qrec)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "Sent keepalive to master\r\n", buf);
    }
    ...
    static void send_heartbeat(qr2_t qrec, int statechanged)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "Sent heartbeat to master (size %d)\r\n",
            buf.len);
        gsDebugBinary(GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_RawDump, buf.buffer, buf.len);
    }
