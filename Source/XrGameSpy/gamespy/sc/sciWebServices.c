...
extern GHTTPBool ghiPostAddFileFromMemory(
    GHTTPPost   post,
    const char* name,
    const char* buffer,
    int         bufferLen,
...
static void sciWsSubmitReportCustom(GHTTPPost thePost, void* theUserData)
{
    ...
    ghiPostAddFileFromMemory(
        thePost, "report", (char*)aWebServices->mSubmitReportData, (gsi_i32)aWebServices->mSubmitReportLength, "report",
        "application/bin");
}
    ...
    void*                     theUserData)
{
    ...
    // Execute soap call
    gsiExecuteSoapCustom(
        scServiceURL, SC_SUBMITREPORT_SOAPACTION, aRequest, sciWsSubmitReportCallback, sciWsSubmitReportCustom,
        theWebServices);
