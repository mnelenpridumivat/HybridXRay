...
#define IGNORED_TXT 's', 'e', 'e', 'd'
    ...
    ********/
    ...
    {
        int        ret;
        const char defaulthost[] = {'k', 'e', 'y', '.', 'g', 'a', 'm', 'e',
                                    's', 'p', 'y', '.', 'c', 'o', 'm', '\0'};   // key.gamespy.com
    ...
    int                                gcd_init_qr2(qr2_t qrec, int gameid)
    {
        ...
        localport =
            (unsigned short)-1; /* we don't process any incoming data ourselves - it gets passed from the QR SDK */
    ...
    void gcd_think(void)
    {
        ...
        char               timeout_t[]   = {'V', 'a', 'l', 'i', 'd', 'a', 't', 'i', 'o', 'n',
                                            ' ', 'T', 'i', 'm', 'e', 'o', 'u', 't', '\0'};   // Validation Timeout
            ...
            while ((node = node->next) != NULL)
            {
                ...
                switch (client->state)
                {
                    ...
                    case cs_gotok:
                        ...
                        client->authfn(
                            products[i].pid, client->localid, 1, client->state == cs_gotok ? validated_t : timeout_t,
                            client->instance);
                    ...
                    case cs_gotnok:
                        ...
                        client->authfn(
                            products[i].pid, client->localid, 0, client->errmsg == NULL ? "" : client->errmsg,
                            client->instance);
                    ...
                    case cs_done:
                        ...
                        if (client->reauthq.next != NULL)
                        {
                            ...
                            while (authnode->next != NULL)
                            {
                                ...
                                {
                                    gsDebugFormat(
                                        GSIDebugCat_CDKey, GSIDebugType_Misc, GSIDebugLevel_Notice,
    ...
    static void send_uon(int skey, const char* ignored, const char* proof, struct sockaddr_in* fromaddr)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_CDKey, GSIDebugType_Network, GSIDebugLevel_Notice,
    ...
    static void send_uoff(int skey, struct sockaddr_in* fromaddr)
    {
        char       outbuf[64];
        int        len;
        const char uoffformat[] = {'\\', 'u', 'o', 'f',  'f', '\\', '\\', 's',
                                   'k',  'e', 'y', '\\', '%', 'd',  '\0'};   //\\uoff\\\\skey\\%d
    ...
    static int get_sockaddrin(char* host, int port, struct sockaddr_in* saddr, struct hostent** savehent)
    {
        ...
        char            broadcast_t[] = {'2', '5', '5', '.', '2', '5', '5', '.',
                                         '2', '5', '5', '.', '2', '5', '5', '\0'};   // 255.255.255.255
