...
static GHTTPBool ghiParseStatus(GHIConnection* connection)
{
    ...
    // Xbox 360 needs "%n" to be manually enabled
    {
        ...
        ////////////////////
        rcode = sscanf(
            connection->recvBuffer.data, "HTTP/%d.%d %d%n", &majorVersion, &minorVersion, &statusCode,
            &statusStringIndex);
    ...
    if ((rcode != 3) ||   // Not all fields read.
                          //!*statusString ||  // No status string.  PANTS|9.16.02 - apparently some servers don't
    ...
    if (connection->chunkedTransfer)
    {
        ...
        while (len > 0)
        {
            ...
            if (connection->chunkReadingState == CRHeader)
            {
                ...
                if (endOfHeader)
                {
                    ...
                    else
                    {
                        ...
                        gsDebugFormat(
                            GSIDebugCat_HTTP, GSIDebugType_Network, GSIDebugLevel_RawDump, "Reading %d byte chunk\n",
            ...
            else if (connection->chunkReadingState == CRChunk)
            {
                ...
                gsDebugFormat(
                    GSIDebugCat_HTTP, GSIDebugType_Network, GSIDebugLevel_RawDump, "Read %d bytes of chunk\n",
                    numBytes);
            ...
            if (location)
            {
                ...
                *end = '\0';
                ...
                if (*location == '/')
                {
                    ...
                    sprintf(
                        connection->redirectURL, "http://%s:%d%s", connection->serverAddress, connection->serverPort,
                        location);
                }
                else
                {
