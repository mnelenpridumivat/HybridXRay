...
int DiscoverReachability(SOCKET sock, unsigned int ip, unsigned short port, int portType)
{
    ...
    gsDebugFormat(
        GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "(%d) Sending ERT Request to %s\n", portType,
...
int DiscoverMapping(SOCKET sock, unsigned int ip, unsigned short port, int portType, int id)
{
    ...
    gsDebugFormat(
        GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "Sending ADDRESS CHECK %d to %s\n", id,
        AddressToString(ip, port, NULL));
...
void OutputMapping(const AddressMapping* theMap)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "Discovered map: [%s] -> [%s]\n",
    ...
    if (sock != INVALID_SOCKET)
    {
        ...
        while (CanReceiveOnSocket(sock))
        {
            ...
            if (ptype == NN_ERTTEST)
            {
                ...
                switch (p.Packet.Init.porttype)
                {
                    case NN_PT_NN1:
                        ...
                        gotERT1 = gsi_true;
                        gsDebugFormat(
                            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment,
            ...
            else if (ptype == NN_ADDRESS_REPLY)
            {
                ...
                gsDebugFormat(
                    GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "Got ADDRESS REPLY %d from: %s\n",
...
gsi_bool DetermineNatType(NAT* nat)
{
    ...
    else
    {
            ...
            (abs(nat->mappings[packet_map3].publicPort - nat->mappings[packet_map2].publicPort) >= 1))
        {
            ...
            nat->ipRestricted && !nat->portRestricted &&
            (abs(nat->mappings[packet_map3].publicPort - nat->mappings[packet_map2].publicPort) >= 1))
        {
            nat->natType     = symmetric;
    ...
    if (nat->mappings[packet_map1a].publicPort == nat->mappings[packet_map1a].privatePort &&
        ...
        nat->mappings[packet_map1a].publicPort == nat->mappings[packet_map2].publicPort &&
        ...
        nat->mappings[packet_map1a].publicPort == nat->mappings[packet_map1a].privatePort &&
