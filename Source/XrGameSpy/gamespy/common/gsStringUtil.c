...
extern "C"
{
    ...
    int _ReadUCS2CharFromUTF8String(const UTF8String theUTF8String, UCS2Char* theUnicodeChar, int theMaxLength)
    {
        ...
        else if (UTF8_IS_TWO_BYTE(theUTF8String[0]))
        {
            ...
            if (UTF8_IS_FOLLOW_BYTE(theUTF8String[1]))
            {
                ...
                *theUnicodeChar =
                    (unsigned short)(((theUTF8String[0] & UTF8_TWO_BYTE_MASK) << 6) + ((theUTF8String[1] & UTF8_FOLLOW_BYTE_MASK)));
                return 2;
            }
        }
        ...
        else if (UTF8_IS_THREE_BYTE(theUTF8String[0]))
        {
            ...
            if (UTF8_IS_FOLLOW_BYTE(theUTF8String[1]) && UTF8_IS_FOLLOW_BYTE(theUTF8String[2]))
            {
                ...
                *theUnicodeChar =
                    (unsigned short)(((theUTF8String[0] & UTF8_THREE_BYTE_MASK) << 12) + ((theUTF8String[1] & UTF8_FOLLOW_BYTE_MASK) << 6) + ((theUTF8String[2] & UTF8_FOLLOW_BYTE_MASK)));
                return 3;
            }
        }
    ...
    int _UCS2CharToUTF8String(UCS2Char theUCS2Char, UTF8String theUTF8String)
    {
        ...
        else if (theUCS2Char <= 0x07FF)
        {
            ...
            // :00000ABC DEFGHIJK
            // :110ABCDE 10FGHIJK
            theUTF8String[0] = (char)(UTF8ByteType)(UTF8_TWO_BYTE_TAG | (theUCS2Char >> 6));   // Store the upper 5/11
                                                                                               // bits as 0x110xxxxx
            theUTF8String[1] =
                (char)(UTF8ByteType)(UTF8_FOLLOW_BYTE_TAG | (theUCS2Char & UTF8_FOLLOW_BYTE_MASK));   // Store the lower
                                                                                                      // 6 bits as
                                                                                                      // 0x10xxxxxx
            return 2;
        }
        else
        {
            ...
            theUTF8String[0] =
                (char)(UTF8ByteType)(UTF8_THREE_BYTE_TAG | (theUCS2Char >> 12));   // Store the upper 4/16 bits as
                                                                                   // 0x1110xxxx
            theUTF8String[1] =
                (char)(UTF8ByteType)(UTF8_FOLLOW_BYTE_TAG | ((theUCS2Char >> 6) & UTF8_FOLLOW_BYTE_MASK));   // Store
                                                                                                             // the
                                                                                                             // 5th-10th
                                                                                                             // bits as
                                                                                                             // 0x10xxxxxx
            theUTF8String[2] =
                (char)(UTF8ByteType)(UTF8_FOLLOW_BYTE_TAG | ((theUCS2Char)&UTF8_FOLLOW_BYTE_MASK));   // Store the last
                                                                                                      // 6 bits as
                                                                                                      // 0x10xxxxxx
            return 3;
        }
    ...
    int _UTF8ToUCS2ConversionLengthOnly(const UTF8String theUTF8String)
    {
        ...
        while (*theReadPos != '\0')
        {
            ...
            // Check for valid three byte string
                ...
                UTF8_IS_THREE_BYTE(theReadPos[0]) && UTF8_IS_FOLLOW_BYTE(theReadPos[1]) &&
