...
extern "C"
{
    ...
    /*
        ***The reason for using a GameSpy assert or custom assert***
         ...
         although assert is handled very gracefully on the windows platform, most consoles do very little of real use
       during assert. Furthermore, the program counter is lost, along with the callstack sometimes. By having a custom
       critical error function, an asm "break" can be set in it, or a debugger break point.  a call stack is immediately
       available.  The error can be drawn onto the screen.  And the choice to ignore can also be given, in order to
       continue stepping through code and further debug.
