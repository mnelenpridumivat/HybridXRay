...
MEM_CHUNK* MEM_CHUNK_POOLAllocChunk(
    MEM_CHUNK_POOL* _this,
    size_t          Size,
    int             Alignment,
...
gsi_bool MEM_CHUNK_POOLItemIsInPoolMemory(MEM_CHUNK_POOL* _this, void* ptr)
{
    ...
    return (
        ((gsi_uint)ptr >= (gsi_uint)MEM_CHUNKMemPtrGet(_this->HeaderStart)) &&
    ...
    while (header)
    {
        ...
        if (!MEM_CHUNKIsFree(header) && (MEM_CHUNKMemTypeGet(header) == type))
        {
            ...
            if ((header != _this->HeaderStart) && (header != _this->HeaderEnd))
            {
                ...
                {
                    gsDebugFormat(
                        GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice, "MemFound ptr:0x%8x  size:%8u %s\n",
...
gsi_u32 gsMemMgrMemHighwaterMarkGet(gsMemMgrContext context)
{
    ...
    gsDebugFormat(
        GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment,
...
void gsMemMgrDumpAllocations()
{
    ...
    while (aBlockPtr != NULL)
    {
        ...
        if (gsiMemBlockIsFlagged(aBlockPtr, BlockFlag_Used))
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "\t[0x%08x] Size: %d\r\n",
    ...
    // Announce finish
    gsDebugFormat(
        GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "\t--%d allocations, %d bytes allocated.\r\n",
        aNumAllocations, aNumBytesAllocated);
    gsDebugFormat(
        GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "\t--%d peak memory usage\r\n",
        gMemoryMgr->mPeakMemoryUsage);

    gsDebugFormat(
        GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "Memory dump complete. (%d ms)\r\n",
