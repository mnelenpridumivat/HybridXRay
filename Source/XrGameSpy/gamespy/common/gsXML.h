...
#define GS_XML_MAX_ATTRIBUTE_VALUE_LENGTH 1024
    ...
    gsi_bool
        gsXmlWriteInt64Element(GSXmlStreamWriter stream, const char* namespaceName, const char* tag, gsi_i64 value);
    ...
    gsi_bool gsXmlWriteBase64BinaryElement(
        ...
        int               len);
    gsi_bool
        gsXmlWriteDateTimeElement(GSXmlStreamWriter stream, const char* namespaceName, const char* tag, time_t value);
    ...
    // gsi_bool gsXmlReadChildAsUnicodeString   (GSXmlStreamReader stream, const char * matchtag, gsi_char ** valueOut,
    // int * lenOut);
    gsi_bool gsXmlReadChildAsUnicodeStringNT(
        GSXmlStreamReader stream,
        const char*       matchtag,
        gsi_char          valueOut[],
        int               maxLen);
    ...
    // NOTE:  Call with NULL valueOut to get the lenOut.  Doing this will not move read position
    gsi_bool
        gsXmlReadChildAsBase64Binary(GSXmlStreamReader stream, const char* matchtag, gsi_u8 valueOut[], int* lenOut);
