...
int SetSockBlocking(SOCKET sock, int isblocking)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "SetSockBlocking: Set socket %d to %s\r\n",
...
#define MAX_IPS sceLIBNET_MAX_INTERFACE
    ...
    for (aInterfaceNum = 0; aInterfaceNum < aNumInterfaces; aInterfaceNum++)
    {
        ...
        int              result = sceInetInterfaceControl(
            &gGSIInsockClientData, &gGSIInsockSocketBuffer, aInterfaceIdArray[aInterfaceNum], sceInetCC_GetAddress,
...
gsi_u32 gsiGetBroadcastIP(void)
{
    ...
    */
    ...
    ip =
        (u32)(addrtbl->bcastAddr[3] | (addrtbl->bcastAddr[2] << 8) | (addrtbl->bcastAddr[1] << 16) | (addrtbl->bcastAddr[0] << 24));
