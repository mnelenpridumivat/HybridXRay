    ...
    if (FS.exist(fn_game, "$level$", "level.game"))
    {
        ...
        if (0 != (O = F->open_chunk(RPOINT_CHUNK)))
        {
            for (int id = 0; O->find_chunk(id); ++id)
            {
                ...
                {
                    if ((GameType == rpgtGameDeathmatch && Type() != GAME_DEATHMATCH) || (GameType == rpgtGameTeamDeathmatch && Type() != GAME_TEAMDEATHMATCH) || (GameType == rpgtGameArtefactHunt && Type() != GAME_ARTEFACTHUNT))
                    {
                        continue;
                    };
                };
...
CSE_Abstract* game_sv_GameState::spawn_begin(LPCSTR N)
{
    ...
    A->s_name                      = N;             // ltx-def
    A->m_gameType.m_GameType.flags = u16(m_type);   // game-type
