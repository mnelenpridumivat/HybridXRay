...
bool game_sv_ArtefactHunt::assign_rp_tmp(game_PlayerState* ps_who, xr_vector<RPoint>& rps, xr_vector<u32>& dest, xr_vector<u32>& rpIDEnemy, xr_vector<u32>& EnemyIt, bool force_find)
{
    dest.clear();
    for (u32 p = 0; p < rps.size(); ++p)
    {
        ...
        bool   Blocked = false;
        m_server->ForEachClientDoSender(
            [&](IClient* client)
            {
...
void game_sv_ArtefactHunt::assign_RP(CSE_Abstract* E, game_PlayerState* ps_who)
{
    ...
    if (rpID.empty() && !rpIDEnemy.empty())
    {
        ...
        SetRP(E, &r);
        //---------------------------------------------------------------------
        IClient* client = m_server->FindClient(
            [&EnemyIt, &PointID](IClient* client)
            {
                if (client->ID == EnemyIt[PointID])
                    return true;
                return false;
            });
