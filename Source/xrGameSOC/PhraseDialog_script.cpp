...
{
    module(L)[class_<CPhrase>("CPhrase").def("GetPhraseScript", &CPhrase::GetPhraseScript),

        class_<CPhraseDialog>("CPhraseDialog").def("AddPhrase", &CPhraseDialog::AddPhrase_script),

        class_<CPhraseScript>("CPhraseScript").def("AddPrecondition", &CPhraseScript::AddPrecondition).def("AddAction", &CPhraseScript::AddAction).def("AddHasInfo", &CPhraseScript::AddHasInfo).def("AddDontHasInfo", &CPhraseScript::AddDontHasInfo).def("AddGiveInfo", &CPhraseScript::AddGiveInfo).def("AddDisableInfo", &CPhraseScript::AddDisableInfo)];
}
