...
void CPHSimpleCharacter::UpdateDynamicDamage(dContact* c, u16 obj_material_idx, dBodyID b, bool bo1)
{
    ...
    {
        SGameMtl* obj_material = GameMaterialLibrary->GetMaterialByIdx(obj_material_idx);
...
{
    if (*p_lastMaterialIDX != u16(-1) && GameMaterialLibrary->GetMaterialByIdx(*p_lastMaterialIDX)->Flags.test(SGameMtl::flPassable) && !b_foot_mtl_check)
        return;
    b_foot_mtl_check = false;
    if (GameMaterialLibrary->GetMaterialByIdx(contact_material_idx)->Flags.test(SGameMtl::flPassable))
