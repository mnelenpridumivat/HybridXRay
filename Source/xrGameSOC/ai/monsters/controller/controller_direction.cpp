...
void CControllerDirection::head_look_point(const Fvector& look_point)
{
    ...
    if (fis_zero(target_dy))
        ...
        bone_speed = _pmt_min_speed + _pmt_rotation_speed * (_abs((x_spine.cur_yaw + x_head.cur_yaw) - (bone_angle_head + bone_angle_torso)) / (2 * (_pmt_head_bone_limit + _pmt_torso_bone_limit)));
    // set motion
