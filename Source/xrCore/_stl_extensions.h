...
public:
    ...
    template<typename C> void destroy(C* p)
    {
        p->~C();
    }
...
public:
    xr_string() {}
    xr_string(const std::basic_string<char, std::char_traits<char>, xalloc<char>>& right)
    {
        assign(right);
    }
    xr_string(const xr_string& right)
    {
        assign(right);
    }
    xr_string(xr_string&& right)
    {
        assign(right);
    }
    xr_string(const char* name)
    {
        assign(name);
    }
    xr_string(int a)
    {
        assign(std::to_string(a).c_str());
    }
    inline xr_string& operator=(const std::basic_string<char, std::char_traits<char>, xalloc<char>>& right)
    {
        assign(right);
        return *this;
    }
    inline xr_string& operator=(const xr_string& right)
    {
        assign(right);
        return *this;
    }
    inline xr_string& operator=(xr_string&& right)
    {
        swap(right);
        return *this;
    }
...
public:
    ...
    /*const_reference operator[]	(size_type _Pos) const { {VERIFY2(_Pos<size(), make_string("index is out of range:
    index requested[%d], size of container[%d]", _Pos, size()).c_str()); } return (*(begin() + _Pos)); } reference
    operator[]		(size_type _Pos)					{ {VERIFY2(_Pos<size(),make_string("index is out of range: index
    requested[%d], size of container[%d]", _Pos, size()).c_str());} return (*(begin() + _Pos)); }*/
};
...
template<typename K, class V, class P = std::less<K>> class xr_map: public std::map<K, V, P>
{
public:
    u32 size() const
    {
...
template<typename K, class V, class P = std::less<K>> class xr_multimap: public std::multimap<K, V, P>
{
public:
    u32 size() const
    {
...
template<typename V, class _HashFcn = std::hash<V>, class _EqualKey = std::equal_to<V>, typename allocator = xalloc<V>> class xr_hash_multiset: public std::hash_multiset<V, _HashFcn, _EqualKey, allocator>
{
public:
    u32 size() const
    {
...
template<typename K, class V, class _HashFcn = std::hash<K>, class _EqualKey = std::equal_to<K>, typename allocator = xalloc<std::pair<K, V>>> class xr_hash_map: public std::hash_map<K, V, _HashFcn, _EqualKey, allocator>
{
public:
    u32 size() const
    {
...
template<typename K, class V, class _HashFcn = std::hash<K>, class _EqualKey = std::equal_to<K>, typename allocator = xalloc<std::pair<K, V>>> class xr_hash_multimap: public std::hash_multimap<K, V, _HashFcn, _EqualKey, allocator>
{
public:
    u32 size() const
    {
...
#else
template<typename K, class V, class _Traits = stdext::hash_compare<K, std::less<K>>> class xr_hash_map: public stdext::hash_map<K, V, _Traits>
{
public:
    u32 size() const
    {
