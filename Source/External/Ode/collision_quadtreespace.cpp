...
void Block::Create(const dVector3 Center, const dVector3 Extents, Block* Parent1, int Depth, Block*& Blocks)
{
    GeomCount    = 0;
    First        = 0;
...
void Block::Collide(dxGeom* g1, dxGeom* g2, void* UserData, dNearCallback* Callback)
{
    ...
    if (Children)
    {
        for (int i = 0; i < SPLITS; i++)
        {
            ...
            {
                if (g1->aabb[AXIS0 * 2 + 0] > Children[i].MaxX || g1->aabb[AXIS0 * 2 + 1] < Children[i].MinX || g1->aabb[AXIS1 * 2 + 0] > Children[i].MaxZ || g1->aabb[AXIS1 * 2 + 1] < Children[i].MinZ)
                    continue;
            }
...
dxQuadTreeSpace::dxQuadTreeSpace(dSpaceID _space, dVector3 Center, dVector3 Extents, int Depth): dxSpace(_space)
{
    ...
    {
        BlockCount += static_cast<int>(powf(static_cast<float>(SPLITS), static_cast<float>(i)));
    }

    Blocks         = (Block*)dAlloc(BlockCount * sizeof(Block));
    Block* Blocks1 = this->Blocks + 1;   // This pointer gets modified!

    this->Blocks[0].Create(Center, Extents, 0, Depth, Blocks1);
...
void dxQuadTreeSpace::collide2(void* UserData, dxGeom* g1, dNearCallback* Callback)
{
    ...
    if (g1->parent_space == this)
    {
        ...
        Block* CurrentBlock1 = (Block*)g1->tome;

        // Collide against block and its children
        CurrentBlock1->Collide(g1, CurrentBlock1->First, UserData, Callback);

        // Collide against parents
        while (true)
        {
            CurrentBlock1 = CurrentBlock1->Parent;
            if (!CurrentBlock1)
            {
                break;
            }
            CurrentBlock1->CollideLocal(g1, UserData, Callback);
        }
    }
    else
