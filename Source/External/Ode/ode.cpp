  ...
  */
    ...
    for (j = w->firstjoint; j; j = (dxJoint*)j->next)
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                int ok = 0;
                for (dxJointNode* n1 = j->node[i].body->firstjoint; n1; n1 = n1->next)
                {
                    if (n1->joint == j)
                        ok = 1;
                }
    ...
    for (b = w->firstbody; b; b = (dxBody*)b->next)
    {
        for (dxJointNode* n1 = b->firstjoint; n1; n1 = n1->next)
        {
            if (&n1->joint->node[0] == n1)
            {
                if (n1->joint->node[1].body != b)
                    dDebug(0, "bad body pointer in joint node of body list (1)");
            }
            else
            {
                if (n1->joint->node[0].body != b)
                    dDebug(0, "bad body pointer in joint node of body list (2)");
            }
            if (n1->joint->tag != count)
...
void dJointAttach(dxJoint* joint, dxBody* body1, dxBody* body2)
{
    ...
    dUASSERT(!((joint->flags & dJOINT_TWOBODIES) && ((body1 != 0) ^ (body2 != 0))), "joint can not be attached to just one body");
