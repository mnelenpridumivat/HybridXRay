...
void dxSimpleSpace::collide(void* data1, dNearCallback* callback)
{
    dAASSERT(callback);
    ...
    for (dxGeom* g1 = first; g1; g1 = g1->next)
    {
        if (GEOM_ENABLED(g1))
        {
            for (dxGeom* g2 = g1->next; g2; g2 = g2->next)
            {
                if (GEOM_ENABLED(g2))
                {
                    collideAABBs(g1, g2, data1, callback);
                }
            }
        }
    }
...
void dxSimpleSpace::collide2(void* data1, dxGeom* geom, dNearCallback* callback)
{
    ...
    for (dxGeom* g = first; g; g = g->next)
    {
        if (GEOM_ENABLED(g))
        {
            collideAABBs(g, geom, data1, callback);
        }
    }
...
static long int prime[NUM_PRIMES] = {1L, 2L, 3L, 7L, 13L, 31L, 61L, 127L, 251L, 509L, 1021L, 2039L, 4093L, 8191L, 16381L, 32749L, 65521L, 131071L, 262139L, 524287L, 1048573L, 2097143L, 4194301L, 8388593L, 16777213L, 33554393L, 67108859L, 134217689L, 268435399L, 536870909L, 1073741789L};
...
{
    if (bounds[0] <= -dInfinity || bounds[1] >= dInfinity || bounds[2] <= -dInfinity || bounds[3] >= dInfinity || bounds[4] <= -dInfinity || bounds[5] >= dInfinity)
    {
        return MAXINT;
    }
...
void dxHashSpace::collide(void* data1, dNearCallback* callback)
{
    ...
    for (geom = first; geom; geom = geom->next)
    {
        ...
        dxAABB* aabb1 = (dxAABB*)ALLOCA(sizeof(dxAABB));
        aabb1->geom   = geom;
        ...
        {
            // aabb1 goes in main list
            aabb1->next  = first_aabb;
            first_aabb   = aabb1;
            aabb1->level = level;
            if (level > maxlevel)
                maxlevel = level;
            ...
            for (i = 0; i < 6; i++)
                aabb1->dbounds[i] = (int)floor(geom->aabb[i] / cellsize);
            // set AABB index
            aabb1->index = n;
            n++;
        }
        ...
        {
            // aabb1 is too big, put it in the big_boxes list. we don't care about
            // setting level, dbounds, index, or the maxlevel
            aabb1->next = big_boxes;
            big_boxes   = aabb1;
        }
    }
    ...
    for (aabb2 = first_aabb; aabb2; aabb2 = aabb2->next)
    {
        ...
        for (int xi = dbounds[0]; xi <= dbounds[1]; xi++)
        {
            for (int yi = dbounds[2]; yi <= dbounds[3]; yi++)
            {
                ...
                {
                    // get the hash index
                    unsigned long hi   = getVirtualAddress(aabb2->level, xi, yi, zi) % sz;
    ...
    for (aabb2 = first_aabb; aabb2; aabb2 = aabb2->next)
    {
        // we are searching for collisions with aabb2
        for (i = 0; i < 6; i++)
            ...
            for (int xi = db[0]; xi <= db[1]; xi++)
            {
                for (int yi = db[2]; yi <= db[3]; yi++)
                {
                    for (int zi = db[4]; zi <= db[5]; zi++)
                    {
                        ...
                        {
                            // node points to an AABB that may intersect aabb2
                            if (node->aabb == aabb2)
                                continue;
                            if (node->aabb->level == level && node->x == xi && node->y == yi && node->z == zi)
                            {
                                // see if aabb2 and node->aabb have already been tested
                                // against each other
                                unsigned char mask;
                                ...
                                if ((tested[i] & mask) == 0)
                                {
                                    collideAABBs(aabb2->geom, node->aabb->geom, data1, callback);
                                }
                                tested[i] |= mask;
                            }
                        }
    ...
    for (aabb2 = first_aabb; aabb2; aabb2 = aabb2->next)
    {
        for (dxAABB* aabb3 = big_boxes; aabb3; aabb3 = aabb3->next)
        {
            collideAABBs(aabb2->geom, aabb3->geom, data, callback);
        }
    }
    /*
  for (aabb=big_boxes; aabb; aabb=aabb->next) {
    ...
    for (aabb2 = big_boxes; aabb2; aabb2 = aabb2->next)
    {
        for (dxAABB* aabb3 = aabb2->next; aabb3; aabb3 = aabb3->next)
        {
            collideAABBs(aabb2->geom, aabb3->geom, data1, callback);
        }
    }
...
void dxHashSpace::collide2(void* data1, dxGeom* geom, dNearCallback* callback)
{
    dAASSERT(geom && callback);
