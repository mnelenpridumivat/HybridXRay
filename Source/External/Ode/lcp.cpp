...
static void swapProblem(ATYPE A, dReal* x, dReal* b, dReal* w, dReal* lo, dReal* hi, int* p, int* state, int* findex, int n, int i1, int i2, int nskip, int do_fast_row_swaps)
{
    dReal tmp;
    int   tmpi;
...
struct dLCP
{
    ...
    dLCP(int _n, int _nub, dReal* _Adata, dReal* _x, dReal* _b, dReal* _w, dReal* _lo, dReal* _hi, dReal* _L, dReal* _d, dReal* _Dell, dReal* _ell, dReal* _tmp, int* _state, int* _findex, int* _p, int* _C, dReal** Arows);
...
dLCP::dLCP(int _n, int _nub, dReal* _Adata, dReal* _x, dReal* _b, dReal* _w, dReal* _lo, dReal* _hi, dReal* _L, dReal* _d, dReal* _Dell, dReal* _ell, dReal* _tmp, int* _state, int* _findex, int* _p, int* _C, dReal** Arows)
{
...
struct dLCP
{
    ...
    void pN_plusequals_ANi(dReal* p, int i, int sign = 1);
    void pC_plusequals_s_times_qC(dReal* p1, dReal s, dReal* q)
    {
        for (int i = 0; i < nC; i++)
            p1[i] += s * q[i];
    }
    void pN_plusequals_s_times_qN(dReal* p1, dReal s, dReal* q)
    {
        for (int i = 0; i < nN; i++)
            p1[i + nC] += s * q[i + nC];
    }
...
void dLCP::pN_equals_ANC_times_qC(dReal* p1, dReal* q)
{
    ...
    for (int i = 0; i < nN; i++)
        p1[i + nC] = dDot(AROW(i + nC), q, nC);
}

void dLCP::pN_plusequals_ANi(dReal* p1, int i, int sign)
{
...
void dLCP::unpermute()
{
    ...
    memcpy(tmp1, x, n * sizeof(dReal));
    for (j = 0; j < n; j++)
        x[p[j]] = tmp1[j];
    memcpy(tmp1, w, n * sizeof(dReal));
    for (j = 0; j < n; j++)
        w[p[j]] = tmp1[j];
}
    ...
    for (i = nub; i < n; i++)
    {
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                for (k = 0; k < lcp.numN(); k++)
                {
                    if ((state[lcp.indexN(k)] == 0 && delta_w[lcp.indexN(k)] < 0) || (state[lcp.indexN(k)] != 0 && delta_w[lcp.indexN(k)] > 0))
                    {
