...
static inline void moveAndRotateBody(dxBody* b, dReal h)
{
    ...
    if (b->flags & dxBodyFlagFiniteRotation)
    {
        ...
        if (b->flags & dxBodyFlagFiniteRotationAxis)
        {
            ...
            dVector3 frv, irv1;   // finite rotation vectori
            dReal    k = dDOT(b->finite_rot_axis, b->avel);
            frv[0]     = b->finite_rot_axis[0] * k;
            frv[1]     = b->finite_rot_axis[1] * k;
            frv[2]     = b->finite_rot_axis[2] * k;
            irv1[0]    = b->avel[0] - frv[0];
            irv1[1]    = b->avel[1] - frv[1];
            irv1[2]    = b->avel[2] - frv[2];
...
void dInternalStepFast(dxWorld* world, dxBody* body[2], dReal* GI[2], dReal* GinvI[2], dxJoint* joint, dxJoint::Info1 info, dxJoint::Info2 Jinfo, dReal stepsize)
{
    (void)world;
    (void)GI;
    int i, j, k;
...
void dInternalStepIslandFast(dxWorld* world, dxBody* const* bodies, int nb, dxJoint** joints, int nj, dReal stepsize, int maxiterations)
{
#ifdef TIMING
