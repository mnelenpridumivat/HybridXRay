...
int              Utils::lerp(int a, int b, int i, int bias, int denom)
{
    ...
    switch (denom)
    {
        ...
        case 15:
            return (a * denom15_weights[denom - i] + b * denom15_weights[i] + round) >> 6;
        case 7:
            return (a * denom7_weights[denom - i] + b * denom7_weights[i] + round) >> 6;
        default:
            nvUnreachable();
            return 0;
    }
...
Vector4 Utils::lerp(Vector4::Arg a, Vector4::Arg b, int i, int bias, int denom)
{
    ...
    switch (denom)
    {
        case 3:
            ...
            i *= 5;   // fall through to case 15
        case 15:
            return (a * float(denom15_weights[denom - i]) + b * float(denom15_weights[i])) / 64.0f;
        case 7:
            return (a * float(denom7_weights[denom - i]) + b * float(denom7_weights[i])) / 64.0f;
        default:
            nvUnreachable();
            return Vector4(0);
    }
