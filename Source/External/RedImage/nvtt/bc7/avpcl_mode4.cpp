...
static void swap_indices(int shapeindex, int indexmode, IntEndptsRGBA endpts[NREGIONS], int indices[NINDEXARRAYS][Tile::TILE_H][Tile::TILE_W])
{
    ...
    for (int region = 0; region < NREGIONS; ++region)
    {
        ...
        if (indices[INDEXARRAY_RGB][y][x] & HIGH_INDEXBIT_RGB(indexmode))
        {
            ...
            int t;
            for (int i = CHANNEL_R; i <= CHANNEL_B; ++i)
            {
                t                   = endpts[region].A[i];
        ...
        if (indices[INDEXARRAY_A][y][x] & HIGH_INDEXBIT_A(indexmode))
        {
            // high bit is set, swap the endpts and indices for this region
            int t;
            for (int i = CHANNEL_A; i <= CHANNEL_A; ++i)
            {
                t                   = endpts[region].A[i];
                endpts[region].A[i] = endpts[region].B[i];
                endpts[region].B[i] = t;
        ...
        for (int x = 0; x < in.size_x; ++x)
        {
            ...
            switch (rotatemode)
            {
                ...
                case ROTATEMODE_RGBA_AGBR:
                    t                  = (out.data[y][x]).x;
                    (out.data[y][x]).x = (out.data[y][x]).w;
                    (out.data[y][x]).w = t;
                    break;
                case ROTATEMODE_RGBA_RABG:
                    t                  = (out.data[y][x]).y;
                    (out.data[y][x]).y = (out.data[y][x]).w;
                    (out.data[y][x]).w = t;
                    break;
                case ROTATEMODE_RGBA_RGAB:
                    t                  = (out.data[y][x]).z;
                    (out.data[y][x]).z = (out.data[y][x]).w;
                    (out.data[y][x]).w = t;
                    break;
                default:
                    nvUnreachable();
...
static float map_colors(const Vector4 colors[], const float importance[], int np, int rotatemode, int indexmode, const IntEndptsRGBA& endpts, const RegionPrec& region_prec, float current_besterr, int indices[NINDEXARRAYS][Tile::TILE_TOTAL])
{
    ...
    for (int i = 0; i < np; ++i)
    {
        ...
        if (AVPCL::flag_premult)
            tile_alpha = (rotatemode == ROTATEMODE_RGBA_AGBR) ? (colors[i]).x : (rotatemode == ROTATEMODE_RGBA_RABG) ? (colors[i]).y : (rotatemode == ROTATEMODE_RGBA_RGAB) ? (colors[i]).z : (colors[i]).w;
        ...
        if (rotatemode == ROTATEMODE_RGBA_RGBA)
        {
            ...
            {
                err = !AVPCL::flag_premult ? Utils::metric3(rgb, palette_rgb[j], rotatemode) : Utils::metric3premult_alphaout(rgb, tile_alpha, palette_rgb[j], palette_alpha);
        ...
        else
        {
            ...
            for (int j = 0; j < NINDICES_RGB(indexmode) && besterr > 0; ++j)
            {
                err = !AVPCL::flag_premult ? Utils::metric3(rgb, palette_rgb[j], rotatemode) : Utils::metric3premult_alphain(rgb, palette_rgb[j], rotatemode);
            ...
            palette_alpha = (rotatemode == ROTATEMODE_RGBA_AGBR) ? (palette_rgb[bestindex]).x : (rotatemode == ROTATEMODE_RGBA_RABG) ? (palette_rgb[bestindex]).y : (rotatemode == ROTATEMODE_RGBA_RGAB) ? (palette_rgb[bestindex]).z : nvCheckMacro(0);
            toterr += besterr;
            ...
            for (int j = 0; j < NINDICES_A(indexmode) && besterr > 0; ++j)
            {
                err = !AVPCL::flag_premult ? Utils::metric1(a, palette_a[j], rotatemode) : Utils::metric1premult(a, tile_alpha, palette_a[j], palette_alpha, rotatemode);
...
static void assign_indices(const Tile& tile, int shapeindex, int rotatemode, int indexmode, IntEndptsRGBA endpts[NREGIONS], const PatternPrec& pattern_prec, int indices[NINDEXARRAYS][Tile::TILE_H][Tile::TILE_W], float toterr[NREGIONS])
{
    Vector3 palette_rgb[NREGIONS][NINDICES3];   // could be nindices2
    float   palette_a[NREGIONS][NINDICES3];     // could be nindices2
    ...
    for (int y = 0; y < tile.size_y; y++)
        for (int x = 0; x < tile.size_x; x++)
        {
            ...
            if (AVPCL::flag_premult)
                tile_alpha = (rotatemode == ROTATEMODE_RGBA_AGBR) ? (tile.data[y][x]).x : (rotatemode == ROTATEMODE_RGBA_RABG) ? (tile.data[y][x]).y : (rotatemode == ROTATEMODE_RGBA_RGAB) ? (tile.data[y][x]).z : (tile.data[y][x]).w;
            ...
            if (rotatemode == ROTATEMODE_RGBA_RGBA)
            {
                ...
                {
                    err = !AVPCL::flag_premult ? Utils::metric3(rgb, palette_rgb[region][i], rotatemode) : Utils::metric3premult_alphaout(rgb, tile_alpha, palette_rgb[region][i], palette_alpha);
            ...
            else
            {
                ...
                for (int i = 0; i < NINDICES_RGB(indexmode) && besterr > 0; ++i)
                {
                    err = !AVPCL::flag_premult ? Utils::metric3(rgb, palette_rgb[region][i], rotatemode) : Utils::metric3premult_alphain(rgb, palette_rgb[region][i], rotatemode);
                ...
                palette_alpha = (rotatemode == ROTATEMODE_RGBA_AGBR) ? (palette_rgb[region][bestindex]).x : (rotatemode == ROTATEMODE_RGBA_RABG) ? (palette_rgb[region][bestindex]).y : (rotatemode == ROTATEMODE_RGBA_RGAB) ? (palette_rgb[region][bestindex]).z : nvCheckMacro(0);
                toterr[region] += besterr;
                ...
                for (int i = 0; i < NINDICES_A(indexmode) && besterr > 0; ++i)
                {
                    err = !AVPCL::flag_premult ? Utils::metric1(a, palette_a[region][i], rotatemode) : Utils::metric1premult(a, tile_alpha, palette_a[region][i], palette_alpha, rotatemode);
...
static float perturb_one(const Vector4 colors[], const float importance[], int np, int rotatemode, int indexmode, int ch, const RegionPrec& region_prec, const IntEndptsRGBA& old_endpts, IntEndptsRGBA& new_endpts, float old_err, int do_b, int indices[NINDEXARRAYS][Tile::TILE_TOTAL])
{
...
static void optimize_endpts(const Tile& tile, int shapeindex, int rotatemode, int indexmode, const float orig_err[NREGIONS], const IntEndptsRGBA orig_endpts[NREGIONS], const PatternPrec& pattern_prec, float opt_err[NREGIONS], IntEndptsRGBA opt_endpts[NREGIONS])
{
