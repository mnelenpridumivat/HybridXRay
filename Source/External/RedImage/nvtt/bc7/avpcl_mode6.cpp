...
static void swap_indices(IntEndptsRGBA_2 endpts[NREGIONS], int indices[Tile::TILE_H][Tile::TILE_W], int shapeindex)
{
    ...
    for (int region = 0; region < NREGIONS; ++region)
    {
        ...
        if (indices[y][x] & HIGH_INDEXBIT)
        {
            ...
            t                    = endpts[region].a_lsb;
            endpts[region].a_lsb = endpts[region].b_lsb;
            endpts[region].b_lsb = t;
...
static float map_colors(const Vector4 colors[], const float importance[], int np, const IntEndptsRGBA_2& endpts, const RegionPrec& region_prec, float current_err, int indices[Tile::TILE_TOTAL])
{
    ...
    for (int i = 0; i < np; ++i)
    {
        ...
        {
            err = !AVPCL::flag_premult ? Utils::metric4(colors[i], palette[j]) : Utils::metric4premult(colors[i], palette[j]);
...
static void assign_indices(const Tile& tile, int shapeindex, IntEndptsRGBA_2 endpts[NREGIONS], const PatternPrec& pattern_prec, int indices[Tile::TILE_H][Tile::TILE_W], float toterr[NREGIONS])
{
    ...
    for (int y = 0; y < tile.size_y; y++)
        for (int x = 0; x < tile.size_x; x++)
        {
            ...
            {
                err = !AVPCL::flag_premult ? Utils::metric4(tile.data[y][x], palette[region][i]) : Utils::metric4premult(tile.data[y][x], palette[region][i]);
...
static float perturb_one(const Vector4 colors[], const float importance[], int np, int ch, const RegionPrec& region_prec, const IntEndptsRGBA_2& old_endpts, IntEndptsRGBA_2& new_endpts, float old_err, int do_b, int indices[Tile::TILE_TOTAL])
{
...
static void optimize_endpts(const Tile& tile, int shapeindex, const float orig_err[NREGIONS], IntEndptsRGBA_2 orig_endpts[NREGIONS], const PatternPrec& pattern_prec, float opt_err[NREGIONS], IntEndptsRGBA_2 opt_endpts[NREGIONS])
{
