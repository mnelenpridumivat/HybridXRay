...
static bool savePNG(Stream& s, const Image* img, const char** tags /*=NULL*/)
{
    ...
    png_set_IHDR(png_ptr, info_ptr, img->width(), img->height(), 8, color_type, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
    ...
    for (uint i = 0; i < img->height(); i++)
    {
        ...
        if (img->format() == Image::Format_RGB)
            row_data[i]--;   // This is a bit of a hack, libpng expects images in ARGB format not BGRA, it supports BGR swapping, but not alpha swapping.
    }
