namespace RedImageTool
{
    ...
    template<typename T> inline T*        red_alloc(size_t count)
    {
        return (T*)MemoryAllocationFunction(nullptr, sizeof(T) * count);
    }
    template<typename T> inline T* red_realloc(T* pointer, size_t count)
    {
        return (T*)MemoryAllocationFunction(pointer, sizeof(T) * count);
    }
    template<typename T> inline void red_free(T* pointer)
    {
