...
/*Divides an integer by a power of two, truncating towards 0.
  ...
  _rmask:    (1<<_shift)-1*/
#define OC_DIV_POW2(_dividend, _shift, _rmask)      ((_dividend) + (OC_SIGNMASK(_dividend) & (_rmask)) >> (_shift))
...
/*Right shifts _dividend by _shift, adding _rval, and subtracting one for
  ...
  When _rval is (1<<_shift-1), this is equivalent to division with rounding
   ties away from zero.*/
#define OC_DIV_ROUND_POW2(_dividend, _shift, _rval) ((_dividend) + OC_SIGNMASK(_dividend) + (_rval) >> (_shift))
...
#define OC_DIV_POW2_RE(_x, _shift)                  ((_x) + ((_x) >> (_shift) & 1) + ((1 << (_shift)) - 1 >> 1) >> (_shift))
...
#define OC_BYTE_TABLE32(_a, _b, _c, _d, _i)                    ((signed char)(((_a) & 0xFF | ((_b) & 0xFF) << 8 | ((_c) & 0xFF) << 16 | ((_d) & 0xFF) << 24) >> (_i) * 8))
/*Accesses one of eight (unsigned) nibbles given an index.
  This can be used to avoid small lookup tables.*/
#define OC_UNIBBLE_TABLE32(_a, _b, _c, _d, _e, _f, _g, _h, _i) ((((_a) & 0xF | ((_b) & 0xF) << 4 | ((_c) & 0xF) << 8 | ((_d) & 0xF) << 12 | ((_e) & 0xF) << 16 | ((_f) & 0xF) << 20 | ((_g) & 0xF) << 24 | ((_h) & 0xF) << 28) >> (_i) * 4) & 0xF)
