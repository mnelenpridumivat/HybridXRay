...
#define BOOST_PP_FILENAME_1       "luabind_memory_manager_generator.h"   // this file
...
namespace luabind
{
    ...
    template<bool> struct luabind_new_detail
    {
        template<typename T BOOST_PP_ENUM_TRAILING_PARAMS(n, class P)> static inline T* initialize(T* result BOOST_PP_ENUM_TRAILING(n, PARAMETERS, BOOST_PP_EMPTY))
        {
            return (new (result) T(BOOST_PP_ENUM_PARAMS(n, p)));
        }
    };

    template<> struct luabind_new_detail<true>
    {
        template<typename T BOOST_PP_ENUM_TRAILING_PARAMS(n, class P)> static inline T* initialize(T* result BOOST_PP_ENUM_TRAILING(n, PARAMETERS, BOOST_PP_EMPTY))
        {
            return (result);
    ...
    {
        template<typename T> static inline T* initialize(T* result, const T& value)
        {
    ...
    template<typename T BOOST_PP_ENUM_TRAILING_PARAMS(n, class P)> inline T* luabind_new(BOOST_PP_ENUM(n, PARAMETERS, BOOST_PP_EMPTY))
    {
