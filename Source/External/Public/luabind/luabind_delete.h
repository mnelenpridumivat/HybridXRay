...
namespace luabind
{
    ...
    template<typename T> inline void                      delete_helper2(T*& pointer, void* top_pointer)
    {
        pointer->~T();
    ...
    template<typename T, bool polymorphic> struct delete_helper
    {
    ...
    template<typename T> struct delete_helper<T, false>
    {
        static inline void apply(T*& pointer)
        {
    ...
    template<typename T> inline void luabind_delete(T*& pointer)
    {
        if (!pointer)
            return;
