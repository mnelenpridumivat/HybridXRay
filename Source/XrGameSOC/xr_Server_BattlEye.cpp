...
BattlEyeServer::BattlEyeServer(xrServer* Server)
{
    ...
    m_succefull = Init(game_version,
        ...
        &PrintMessage, &SendPacket, &KickPlayer, &pfnExit, &pfnRun, &pfnCommand, &pfnAddPlayer, &pfnRemovePlayer, &pfnNewPacket);
    ...
    for (u32 it = 0; it < cnt; ++it)
    {
        ...
        if (l_pC->ID.value() == (u32)player)
        {
            ...
            else
            {
                ...
                string512 reason2;
                sprintf_s(reason2, sizeof(reason2), "  Disconnecting : %s !  Server's Client kicked by BattlEye Server.  Reason: %s", l_pC->ps->getName(), reason);
                Msg(reason2);
