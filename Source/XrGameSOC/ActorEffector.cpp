...
BOOL CAnimatorCamLerpEffector::Process(Fvector& p, Fvector& d, Fvector& n, float& fFov, float& fFar, float& fAspect)
{
    ...
    Args.fFar    = fFar;
    Args.fAspect = fAspect;
    if (!inherited::inherited::ProcessCam(Args))
        return FALSE;
...
CControllerPsyHitCamEffector::CControllerPsyHitCamEffector(ECamEffectorType type, const Fvector& src_pos, const Fvector& target_pos, float time): inherited(eCEControllerPsyHit, flt_max)
{
    m_time_total   = time;
    m_time_current = 0;
