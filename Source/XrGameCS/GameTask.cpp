...
void CGameTask::CreateMapLocation(bool on_load)
{
    ...
    {
        if (Level().MapManager().GetMapLocation(m_map_location, m_map_object_id))
        {
            VERIFY2(0, make_string("Can`t create duplicated MapLocation of GameTask: task_id=%s,  map_location=%s,  object_id=%d", m_ID.c_str(), m_map_location.c_str(), m_map_object_id));
