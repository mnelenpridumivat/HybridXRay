            ...
            if (no_last_pos)
                for (i = pos_tries.begin(); pos_tries.end() != i; ++i)
                {
                    ...
                    if (TriContainPoint((dReal*)&V_array[i->T->verts[0]], (dReal*)&V_array[i->T->verts[1]], (dReal*)&V_array[i->T->verts[2]], i->norm, i->side0, i->side1, p))
                        if (!((dDOT(neg_tri.norm, (dReal*)&V_array[i->T->verts[0]]) - neg_tri.pos) > 0.f) || !((dDOT(neg_tri.norm, (dReal*)&V_array[i->T->verts[1]]) - neg_tri.pos) > 0.f) || !((dDOT(neg_tri.norm, (dReal*)&V_array[i->T->verts[2]]) - neg_tri.pos) > 0.f))
                        {
                            include = false;
                            break;
                        }
        ...
        if (b_neg_depth < dInfinity)
        {
            ...
            if (no_last_pos)
                for (i = pos_tries.begin(); pos_tries.end() != i; ++i)
                {
                    ...
                    if (!((dDOT(b_neg_tri.norm, (dReal*)&V_array[i->T->verts[0]]) - b_neg_tri.pos) > 0.f) || !((dDOT(b_neg_tri.norm, (dReal*)&V_array[i->T->verts[1]]) - b_neg_tri.pos) > 0.f) || !((dDOT(b_neg_tri.norm, (dReal*)&V_array[i->T->verts[2]]) - b_neg_tri.pos) > 0.f)
