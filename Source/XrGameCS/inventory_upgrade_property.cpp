...
namespace inventory
{
    namespace upgrade
    {
        ...
        void Property::construct(shared_str const& property_id, Manager& manager_r)
        {
            ...
            R_ASSERT2(ai().script_engine().functor(functor_str, m_desc.functr), make_string("Failed to get upgrade property functor in section[%s], functor[%s]", id_str(), functor_str));
            m_desc();   // test
