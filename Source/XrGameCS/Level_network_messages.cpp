...
#pragma todo("remove next deadlock checking after testing...")
            ...
            case M_CHANGE_LEVEL_GAME:
            {
                ...
                else
                {
                    ...
                    sprintf_s(NewServerOptions, "%s/%s/%s%s", LevelName.c_str(), GameType.c_str(), map_ver_string, LevelVersion.c_str());

                    if (m_SO)
                    {
                        string4096 additional_options;
                        strcat_s(NewServerOptions, sizeof(NewServerOptions), remove_version_option(m_SO, additional_options, sizeof(additional_options)));
                    }
