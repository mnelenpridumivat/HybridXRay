...
void game_cl_CaptureTheArtefact::shedule_Update(u32 dt)
{
    ...
    if ((Level().IsDemoPlayStarted() || Level().IsDemoPlayFinished()) && m_game_ui)
    {
        ...
        STRCONCAT(demo_play_string, "demo play active : ", itoa(int(Level().GetDemoPlayPos() * 100), tmp_buf1, 10), "%%, play speed: ", itoa(int(Level().GetDemoPlaySpeed()), tmp_buf2, 10), "x");
    ...
    switch (msg)
    {
        ...
        case GAME_EVENT_ARTEFACT_TAKEN:
        {
            ...
            if (ps->team == artefactOwnerTeam)
            {
                ...
                sprintf_s(Text, "%s%s %s%s", CTeamInfo::GetTeam_color_tag(ModifyTeam(artefactOwnerTeam) + 1), ps->name, Color_Main, st.translate("mp_returned_artefact").c_str());
                PlayReturnedTheArtefact(ps);
            }
            else if (ps != local_player)
            {
                // player has captured the artefact
                sprintf_s(Text, "%s%s %s%s", CTeamInfo::GetTeam_color_tag(ModifyTeam(ps->team) + 1), ps->name, Color_Main, st.translate("mp_captured_artefact").c_str());
            ...
            {
                sprintf_s(Text, "%s%s", Color_Main, *st.translate("mp_you_captured_artefact"));
        ...
        case GAME_EVENT_ARTEFACT_DROPPED:   //ahunt
        {
            ...
            if (ps)
            {
                sprintf_s(Text, "%s%s %s%s", CTeamInfo::GetTeam_color_tag(ModifyTeam(ps->team) + 1), ps->name, Color_Main,
                    st.translate("mp_has_dropped_artefact").c_str());   //need translate
            }
            else
            {
                sprintf_s(Text, "%s%s", Color_Main, st.translate("mp_artefact_dropped").c_str());
            }
            CommonMessageOut(Text);
        ...
        case GAME_EVENT_ARTEFACT_ONBASE:
        {
            ...
            {
                //artefact on base !
                sprintf_s(Text, "%s%s", Color_Artefact, st.translate("mp_artefact_on_base").c_str());
            }
            else
            {
                //artefact on enemy base !
                sprintf_s(Text, "%s%s", Color_Artefact, st.translate("mp_artefact_on_enemy_base").c_str());
            }
            CommonMessageOut(Text);
