...
bool game_sv_GameState::NewPlayerName_Exists(void* pClient, LPCSTR NewName)
{
    ...
    struct client_finder
    {
        ...
        bool     operator()(IClient* client)
        {
            if (client == CL)
                return false;
            if (!xr_strcmp(NewName, client->name.c_str()))
                return true;
            return false;
        }
    };
    ...
    if (!tmp_predicate.CL->name || xr_strlen(tmp_predicate.CL->name.c_str()) == 0)
        return false;
...
void game_sv_GameState::AddDelayedEvent(NET_Packet& tNetPacket, u16 type, u32 time, ClientID sender)
{
    ...
    if ((type == GAME_EVENT_PLAYER_STARTED) || (type == GAME_EVENT_PLAYER_READY) || (type == GAME_EVENT_VOTE_START) || (type == GAME_EVENT_VOTE_YES) || (type == GAME_EVENT_VOTE_NO) || (type == GAME_EVENT_PLAYER_AUTH))
    {
        m_event_queue->Create(tNetPacket, type, time, sender);
    }
    else
