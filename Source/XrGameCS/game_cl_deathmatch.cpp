    ...
    switch (Phase())
    {
        case GAME_PHASE_INPROGRESS:
        {
            ...
            if (local_player && !local_player->IsSkip())
            {
                ...
                if (Level().CurrentEntity() && smart_cast<CSpectator*>(Level().CurrentEntity()))
                {
                    if (!(pCurBuyMenu && pCurBuyMenu->IsShown()) && !(pCurSkinMenu && pCurSkinMenu->IsShown()) && !(m_game_ui->m_pMapDesc && m_game_ui->m_pMapDesc->IsShown()) && (HUD().GetUI() && HUD().GetUI()->GameIndicatorsShown()))
                    {
                        if (!m_bSkinSelected)
                ...
                if (Level().CurrentControlEntity() && smart_cast<CSpectator*>(Level().CurrentControlEntity()) && (HUD().GetUI() && HUD().GetUI()->GameIndicatorsShown()))
                {
