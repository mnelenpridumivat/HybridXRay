...
#include "../External/Public/crypto/crypto.h"
...
bool game_cl_mp::OnKeyboardPress(int key)
{
    ...
    if ((game_phase != GAME_PHASE_INPROGRESS) && (kQUIT != key) && (kCONSOLE != key) && (kCHAT != key) && (kVOTE_BEGIN != key) && (kVOTE != key) && (kVOTEYES != key) && (kVOTENO != key))
    {
        return true;
    }
...
void game_cl_mp::OnPlayerVoted(game_PlayerState* ps)
{
    ...
    CStringTable st;
    string1024   resStr;
    sprintf_s(resStr, "%s\"%s\" %s%s %s\"%s\"", Color_Teams[ps->team], ps->getName(), Color_Main, *st.translate("mp_voted"), ps->m_bCurrentVoteAgreed ? Color_Green : Color_Red, *st.translate(ps->m_bCurrentVoteAgreed ? "mp_voted_yes" : "mp_voted_no"));
...
{
    sprintf_s(file_name, "%02d%02d%02d-%02d%02d%02d_%s", date_time.wYear % 100, date_time.wMonth, date_time.wDay, date_time.wHour, date_time.wMinute, date_time.wSecond, file_suffix);
}
