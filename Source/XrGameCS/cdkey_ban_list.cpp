...
void cdkey_ban_list::ban_player(xrClientData const* player_data, s32 end_time_sec, xrClientData const* admin)
{
    ...
    if (player_data->ps)
        tmp_client->client_name = player_data->ps->name;
    else
        tmp_client->client_name = player_data->name;
    ...
    if (admin)
    {
        ...
        if (admin->ps)
            tmp_client->admin_name = admin->ps->name;
        else
            tmp_client->admin_name = admin->name;
    }
    else
    {
...
char const* print_time(time_t const& src_time, string64& dest_time)
{
    ...
    sprintf_s(dest_time, sizeof(dest_time), "%02d.%02d.%d_%02d:%02d:%02d", tmp_tm->tm_mday, tmp_tm->tm_mon + 1, tmp_tm->tm_year + 1900, tmp_tm->tm_hour, tmp_tm->tm_min, tmp_tm->tm_sec);
    return dest_time;
}
...
void cdkey_ban_list::print_ban_list(char const* filter_string)
{
    ...
    {
        string64 temp_time;
        sprintf_s(tmp_string, "- (player index : %d), (ip : %s), (name : %s), (end time : %s), (hex digest : %s);", index, (*i)->client_ip_addr.to_string().c_str(), (*i)->client_name.c_str(), print_time((*i)->ban_end_time, temp_time), (*i)->client_hexstr_digest.c_str());
        if (filter_string)
        {
