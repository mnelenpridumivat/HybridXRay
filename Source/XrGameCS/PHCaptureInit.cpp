...
{
    if (!a_taget_object || !a_taget_object->m_pPhysicsShell || !a_taget_object->m_pPhysicsShell->isActive() || smart_cast<CInventoryItem*>(a_taget_object) || !a_character || !a_character->b_exist || !a_character->PhysicsRefObject() || !a_character->PhysicsRefObject()->Visual())
        return false;
...
{
    if (!can_capture(a_character, a_taget_object) || a_taget_element == BI_NONE || !a_taget_object->Visual())
        return false;
