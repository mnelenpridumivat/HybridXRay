...
void CAI_Bloodsucker::Load(LPCSTR section)
{
    ...
    m_critical_hit_chance                = READ_IF_EXISTS(pSettings, r_float, section, "critical_hit_chance", default_critical_hit_chance);

    m_critical_hit_camera_effector_angle = READ_IF_EXISTS(pSettings, r_float, section, "critical_hit_camera_effector_angle", default_critical_hit_camera_effector_angle);

    m_camera_effector_move_angular_speed = READ_IF_EXISTS(pSettings, r_float, section, "camera_effector_move_angular_speed", default_camera_effector_move_angular_speed);

    m_invisibility_activate_delay        = READ_IF_EXISTS(pSettings, r_u32, section, "invisibility_activate_delay", default_invisibility_activate_delay);
}
...
void CAI_Bloodsucker::apply_actor_camera_effector()
{
    if (!m_camera_effector_start_move_tick || m_camera_effector_last_move_tick >= m_camera_effector_start_move_tick + m_camera_effector_move_time_milis)
    {
        return;
    }
    ...
    const TTime current_tick         = _min(time(), m_camera_effector_start_move_tick + m_camera_effector_move_time_milis);

    const float relative_time1       = (m_camera_effector_last_move_tick - m_camera_effector_start_move_tick) / (float)m_camera_effector_move_time_milis;

    const float relative_time2       = (current_tick - m_camera_effector_start_move_tick) / (float)m_camera_effector_move_time_milis;
    ...
    Actor()->cam_Active()->Move(m_camera_effector_hor_angle > 0 ? kRIGHT : kLEFT, _abs(m_camera_effector_hor_angle) * angle_relative);

    Actor()->cam_Active()->Move(m_camera_effector_ver_angle > 0 ? kUP : kDOWN, _abs(m_camera_effector_ver_angle) * angle_relative);
}
...
void CAI_Bloodsucker::HitEntity(const CEntity* pEntity, float fDamage, float impulse, Fvector& dir)
{
    ...
    if (is_critical)
    {
        ...
        if (pEntity == Actor())
        {
            m_last_critical_hit_tick = m_camera_effector_start_move_tick = m_camera_effector_last_move_tick = time();
