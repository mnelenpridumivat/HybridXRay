...
void game_cl_TeamDeathmatch::TranslateGameMessage(u32 msg, NET_Packet& P)
{
    ...
    switch (msg)
    {
        ...
        case PLAYER_CHANGE_TEAM:   //tdm
        {
            ...
            sprintf_s(Text, "%s%s %s%s %s%s", CTeamInfo::GetTeam_color_tag(int(OldTeam)), pPlayer->name, Color_Main, *st.translate("mp_switched_to"), CTeamInfo::GetTeam_color_tag(int(NewTeam)), CTeamInfo::GetTeam_name(int(NewTeam)));
            CommonMessageOut(Text);
            //---------------------------------------
