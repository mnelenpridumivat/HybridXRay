...
cover::cover(smart_cover::object const& object, DescriptionPtr description, bool const& is_combat_cover): inherited(object.Position(), object.ai_location().level_vertex_id()), m_object(object), m_description(description), m_id(m_object.cName()), m_is_combat_cover(is_combat_cover)
{
    m_is_smart_cover         = 1;
    ...
    for (; I != E; ++I, ++i)
    {
        ...
        VERIFY2(graph.valid_vertex_id(level_vertex_id), make_string("invalid vertex id: smart cover[%s], loophole [%s]", m_object.cName().c_str(), (*I)->id().c_str()));
        vertex(**I, (*i).second);
