...
void CUI3tButton::Update()
{
    ...
    if (m_bTextureEnable)
    {
        if (!m_bIsEnabled)
        {
            ...
            {
                m_back_frameline->SetState(S_Disabled);
            }
        }
        else if (CUIButton::BUTTON_PUSHED == m_eButtonState)
        {
            ...
            {
                m_back_frameline->SetState(S_Touched);
            }
        }
        else if (m_bCursorOverWindow)
        {
            ...
            {
                m_back_frameline->SetState(S_Highlighted);
            }
        }
        else
        {
            ...
            {
                m_back_frameline->SetState(S_Enabled);
            }
        }
    }
...
void CUI3tButtonEx::Show(bool status)
{
    ...
    if (m_hint_owner)
        m_hint_owner->Show(status);
}

bool CUI3tButtonEx::OnMouseDown(int mouse_btn)
{
    if (m_hint_owner)
        m_hint_owner->disable_hint();
