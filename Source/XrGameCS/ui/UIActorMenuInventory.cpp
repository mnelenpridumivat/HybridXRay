...
{
    CUIDragDropListEx* all_lists[] = {m_pInventoryBeltList, m_pInventoryPistolList, m_pInventoryAutomaticList, m_pInventoryOutfitList, m_pInventoryDetectorList, m_pInventoryBagList, m_pTradeActorBagList, m_pTradeActorList, NULL};
...
void CUIActorMenu::PropertiesBoxForSlots(PIItem item, bool& b_show)
{
    ...
    if (!pOutfit && cur_slot != NO_ACTIVE_SLOT && !inv->m_slots[cur_slot].m_bPersistent && inv->CanPutInSlot(item))
    {
    ...
    if (item->Ruck() && inv->CanPutInRuck(item) && (cur_slot == (u32)(-1) || !inv->m_slots[cur_slot].m_bPersistent))
    {
        if (!pOutfit)
        {
