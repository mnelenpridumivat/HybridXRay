...
{
    if (CActor::actor_camera_shell && CActor::actor_camera_shell->get_ElementByStoreOrder(0)->PhysicsRefObject() != l_actor)
...
void set_camera_collision(const Fvector& box_size, const Fmatrix& xform, CPhysicsElement* roote, CBoxGeom* box)
{
    ...
    const Fvector character_collision_box_size = Fvector().add(box_size, Fvector().set(camera_collision_character_sckin_depth, camera_collision_character_sckin_depth, camera_collision_character_sckin_depth));
