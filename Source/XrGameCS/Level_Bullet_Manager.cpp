...
void SBullet::Init(const Fvector& position, const Fvector& direction, float starting_speed, float power, float power_critical, float impulse, u16 sender_id, u16 sendersweapon_id, ALife::EHitType e_hit_type, float maximum_distance, const CCartridge& cartridge, bool SendHit)
{
    flags._storage = 0;
    bullet_pos     = position;
...
void CBulletManager::AddBullet(const Fvector& position, const Fvector& direction, float starting_speed, float power, float power_critical, float impulse, u16 sender_id, u16 sendersweapon_id, ALife::EHitType e_hit_type, float maximum_distance, const CCartridge& cartridge, bool SendHit, bool AimBullet)
{
    //.	m_Lock.Enter				();
...
static Fvector trajectory_position(Fvector const& start_position, Fvector const& start_velocity, Fvector const& gravity, float const air_resistance, float const time)
{
    if (air_resistance * (time + air_resistance_epsilon) <= 1.f)
    {
        Fvector const xz_velocity = Fvector().set(start_velocity.x, 0.f, start_velocity.z);
    ...
    float const parabolic_time     = _max(0.f, 1.f / air_resistance - air_resistance_epsilon);
    float const fall_down_time     = time - parabolic_time;
    Fvector     parabolic_position = ::parabolic_position(start_position, start_velocity, gravity, air_resistance, parabolic_time);
    Fvector     parabolic_velocity = ::parabolic_velocity(start_velocity, gravity, air_resistance, parabolic_time);
    return (parabolic_position.mad(parabolic_velocity, fall_down_time).mad(gravity, _sqr(fall_down_time) * .5f));
}
...
static bool try_update_bullet(SBullet& bullet, Fvector const& gravity, float const air_resistance, float const time)
{
    ...
    if ((bullet.bullet_pos.x < level_box.x1) || (bullet.bullet_pos.x > level_box.x2) || (bullet.bullet_pos.y < level_box.y1) ||
