...
bool CInventory::Slot(PIItem pIItem, bool bNotActivate, bool strict_placement)
{
    ...
    {
#ifdef _DEBUG
        Msg("there is item %s[%d,%x] in slot %d[%d,%x]", *m_slots[pIItem->GetSlot()].m_pIItem->object().cName(), m_slots[pIItem->GetSlot()].m_pIItem->object().ID(), m_slots[pIItem->GetSlot()].m_pIItem, pIItem->GetSlot(), pIItem->object().ID(), pIItem);
#endif
...
bool CInventory::Action(s32 cmd, u32 flags)
{
    ...
    if (m_iActiveSlot < m_slots.size() && m_slots[m_iActiveSlot].m_pIItem && !m_slots[m_iActiveSlot].m_pIItem->object().getDestroy() && m_slots[m_iActiveSlot].m_pIItem->Action(cmd, flags))
        return true;
...
*/
    if (OnServer())
    {
        if (m_iActiveSlot != m_iNextActiveSlot)
        {
            ...
            if (Level().CurrentViewEntity() == pActor_owner)
            {
                if ((m_iNextActiveSlot != NO_ACTIVE_SLOT) && m_slots[m_iNextActiveSlot].m_pIItem && !g_player_hud->allow_activation(m_slots[m_iNextActiveSlot].m_pIItem->cast_hud_item()))
                    return;
            }
