...
void CPatrolPathManager::select_point(const Fvector& position, u32& dest_vertex_id)
{
    ...
    if (!actual() || !m_path->vertex(m_curr_point_index))
    {
        ...
        R_ASSERT2(vertex || show_restrictions(m_object), make_string("any vertex in patrol path [%s] is inaccessible for object [%s]", *m_path_name, *m_game_object->cName()));
        R_ASSERT2(ai().level_graph().valid_vertex_id(vertex->data().level_vertex_id()), make_string("patrol path[%s], point on path [%s],object [%s]", *m_path_name, *vertex->data().name(), *m_game_object->cName()));
