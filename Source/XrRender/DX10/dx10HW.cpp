...
void CHW::CreateDevice(HWND m_hWnd, bool move_window)
{
    ...
    */
    ...
    if (GDebugRender)
    {
        ID3D10InfoQueue* InfoQueue = nullptr;
        if (SUCCEEDED(pDevice->QueryInterface(&InfoQueue)))
        {
            InfoQueue->SetBreakOnSeverity(D3D10_MESSAGE_SEVERITY_CORRUPTION, TRUE);
            InfoQueue->SetBreakOnSeverity(D3D10_MESSAGE_SEVERITY_ERROR, TRUE);
            InfoQueue->SetBreakOnSeverity(D3D10_MESSAGE_SEVERITY_WARNING, FALSE);
