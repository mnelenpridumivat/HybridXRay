...
void gradient::fill(::XrWeatherEditor::environment::manager& environment, LPCSTR name, LPCSTR description, ::XrWeatherEditor::property_holder& holder)
{
    ...
    m_property_holder->add_property("opacity", "properties", "this option is resposible for thunderbolt gradient opacity", fOpacity, fOpacity);
    m_property_holder->add_property("minimum radius", "properties", "this option is resposible for thunderbolt gradient minimum radius", fRadius.x, fRadius.x);
    m_property_holder->add_property("maximum _radius", "properties", "this option is resposible for thunderbolt gradient maximum radius", fRadius.y, fRadius.y);
    ...
    m_property_holder->add_property("shader", "properties", "this option is resposible for thunderbolt gradient shader", shader.c_str(), string_getter, string_setter, &*environment.shader_ids().begin(), environment.shader_ids().size(), XrWeatherEditor::property_holder::value_editor_tree_view, XrWeatherEditor::property_holder::cannot_enter_text);

    string_getter.bind(this, &gradient::texture_getter);
    string_setter.bind(this, &gradient::texture_setter);
    m_property_holder->add_property("texture", "", "this option is resposible for thunderbolt gradient texture", texture.c_str(), texture, ".dds", "Texture files (*.dds)|*.dds", detail::real_path("$game_textures$", "").c_str(), "Select texture...", XrWeatherEditor::property_holder::cannot_enter_text, XrWeatherEditor::property_holder::remove_extension);
}
