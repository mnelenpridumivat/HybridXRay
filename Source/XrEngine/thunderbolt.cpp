...
shared_str
    CEffect_Thunderbolt::AppendDef(IEnvironment& environment, CInifile* pIni, CInifile* thunderbolts, LPCSTR sect)
{
#pragma FIXME("Panda: я это сломал!")
    if (!sect || (0 == sect[0]))
        return "";
...
#pragma FIXME("Panda: я это сломал!")
    /*	VERIFY					(id.size());
        ...
        current		            = g_pGamePersistent->Environment().thunderbolt_collection(collection,
       id)->GetRandomDesc(); VERIFY(current);
...
#pragma FIXME("Panda: я это сломал!")
    ...
    if (state == stWorking)
    {
        ...
        fClr.set(
            clampr(float(color_get_R(uClr) / 255.f), 0.f, 1.f), clampr(float(color_get_G(uClr) / 255.f), 0.f, 1.f),
        ...
        if (::Render->get_generation()==IRender_interface::GENERATION_R2)	{
            ...
            VERIFY2(g_pGamePersistent->Environment().CurrentEnv->sun_dir.y<0,"Invalid sun direction settings while
        CEffect_Thunderbolt");
