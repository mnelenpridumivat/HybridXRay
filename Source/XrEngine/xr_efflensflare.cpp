...
#pragma FIXME("Panda: я это сломал!")
    ...
    //	LFState			previous_state = m_State;
        ...
        case lfsShow:
            m_StateBlend 	= m_Current?(m_StateBlend + m_Current->m_StateBlendUpSpeed * Device->fTimeDelta *
    tf):1.f+EPS; if (m_StateBlend>=1.f) m_State=lfsIdle; break; case lfsHide: m_StateBlend 	= m_Current?(m_StateBlend -
    m_Current->m_StateBlendDnSpeed * Device->fTimeDelta * tf):0.f-EPS; if (m_StateBlend<=0.f){ m_State		= lfsShow;
                m_Current	= desc;
    ...
    state_to_string(m_State)); clamp(m_StateBlend,0.f,1.f);
