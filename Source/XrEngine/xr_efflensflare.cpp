...
void CLensFlare::OnFrame(shared_str id)
{
    ...
    {
#if 0
        float dist = EditorScene->ZFar();
        // if (EditorScene->RayPick(Device->vCameraPosition, vSunDir, dist))
        if (EditorScene->RayPick(EditorScene->GetCameraPosition(), vSunDir, dist))
