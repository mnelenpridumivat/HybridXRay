...
//-----------------------------------------------------------------------------
void	CEnvSOCModifier::load	(IReader* fs, u32 version)
{
//	Fvector			dummy;
...
float	CEnvSOCModifier::sum	(IEnvModifier& M, Fvector3& view)
{
...
// Environment ambient
//-----------------------------------------------------------------------------
void CEnvSOCAmbient::load(const shared_str& sect)
{
}
...
//-----------------------------------------------------------------------------
CEnvSOCDescriptor::CEnvSOCDescriptor()
{
	exec_time			= 0.0f;
...
void CEnvSOCDescriptor::load	(LPCSTR exec_tm, LPCSTR S, IEnvironment* parent)
{
	Ivector3 tm				={0,0,0};
...
void CEnvSOCDescriptor::on_device_create	()
{
	m_pDescriptor->OnDeviceCreate(*this);
}

void CEnvSOCDescriptor::on_device_destroy	()
{
...
// Environment Mixer
//-----------------------------------------------------------------------------
void CEnvSOCDescriptorMixer::destroy()
{
	m_pDescriptorMixer->Destroy();
	on_device_destroy();
}

void CEnvSOCDescriptorMixer::clear	()
{
...
// Environment IO
//-----------------------------------------------------------------------------
IEnvAmbient* CEnvironmentSOC::AppendEnvAmb(const shared_str& sect)
{
	for (EnvAmbVecIt it = Ambients.begin(); it != Ambients.end(); it++)
		if ((*it)->name().equal(sect)) return *it;
	auto Ambient = xr_new<CEnvSOCAmbient>();
...
void	CEnvironmentSOC::mods_load			()
{
	...
	if (FS.exist(path,"$level$","level.env_mod"))	
	{
		...
		{
			CEnvSOCModifier* E = xr_new< CEnvSOCModifier>();
			E->load				(fs,0);
			...
			if (pSettings->r_line("weathers",w_idx,&weather,&sect_w)){
				...
				for (int env_idx=0; env_idx<env_count; env_idx++){
					if (pSettings->r_line(sect_w,env_idx,&exec_tm,&sect_e)){
						CEnvSOCDescriptor*		D=xr_new<CEnvSOCDescriptor>();
	...
	if (WeatherFXs.empty()){
		...
		for (int w_idx=0; w_idx<line_count; w_idx++){
			...
			if (pSettings->r_line("weather_effects",w_idx,&weather,&sect_w)){
				...
				env.push_back	(xr_new<CEnvSOCDescriptor>()); env.back()->exec_time_loaded = 0;
				env.push_back	(xr_new<CEnvSOCDescriptor>()); env.back()->exec_time_loaded = 0;
				int env_count	= pSettings->line_count(sect_w);
				LPCSTR exec_tm, sect_e;
				for (int env_idx=0; env_idx<env_count; env_idx++){
				...
				env.push_back	(xr_new<CEnvSOCDescriptor>()); env.back()->exec_time_loaded = DAY_LENGTH;
			}
		}
		// sorting weather envs
