...
void CGameGraph::script_register(lua_State* L)
{
    module(L)[def("game_graph", &get_game_graph),
        ...
        class_<IGameGraph>("IGameGraph").def("accessible", &get_accessible1).def("accessible", &get_accessible2).def("valid_vertex_id", &IGameGraph::valid_vertex_id).def("vertex", &IGameGraph::vertex).def("vertex_id", &IGameGraph::vertex_id),
