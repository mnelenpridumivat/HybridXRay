...
void xrServer::ProcessClientDigest(xrClientData* xrCL, NET_Packet* P)
{
    ...
    if (server_game->IsPlayerBanned(xrCL->m_cdkey_digest.c_str(), admin_name) || server_game->IsPlayerBanned(secondary_cdkey.c_str(), admin_name))
    {
        R_ASSERT2(tmp_client != GetServerClient(), "can't disconnect server client");
        Msg("--- Client [%s][%s] tried to connect - rejecting connection (he is banned by %s) ...", tmp_client->m_cAddress.to_string().c_str(), tmp_client->name.c_str(), admin_name.size() ? admin_name.c_str() : "Server");
