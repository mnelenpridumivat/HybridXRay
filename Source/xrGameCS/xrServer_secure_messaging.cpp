...
{
    VERIFY(xrCL);
    s32 new_seed = m_seed_generator.genrate();
    ...
    key_sync_command.w_s32(new_seed);

    SendTo(xrCL->ID, key_sync_command, net_flags(TRUE, TRUE, TRUE, TRUE));
}
...
void xrServer::OnSecureMessage(NET_Packet& P, xrClientData* xrClSender)
{
    ...
    strcpy_s(dbg_tmp_buff, "xray crypt check");
