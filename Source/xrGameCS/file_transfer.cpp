...
filetransfer_node::filetransfer_node(shared_str const& file_name, u32 const chunk_size, sending_state_callback_t const& callback): m_writer_as_src(NULL), m_file_name(file_name), m_is_reader_memory(false), m_writer_max_size(0), m_chunk_size(chunk_size), m_last_peak_throughput(0), m_last_chunksize_update_time(0), m_user_param(0), m_process_callback(callback)
{
    m_reader = FS.r_open(file_name.c_str());
}

filetransfer_node::filetransfer_node(u8* data, u32 const data_size, u32 const chunk_size, sending_state_callback_t const& callback, u32 user_param): m_writer_as_src(NULL), m_is_reader_memory(true), m_writer_max_size(0), m_chunk_size(chunk_size), m_last_peak_throughput(0), m_last_chunksize_update_time(0), m_user_param(user_param), m_process_callback(callback)
{
    m_reader = xr_new<IReader>(static_cast<void*>(data), static_cast<int>(data_size));
}
filetransfer_node::filetransfer_node(CMemoryWriter* m_src_writer, u32 const max_size, u32 const chunk_size, sending_state_callback_t const& callback, u32 user_param): m_reader(NULL), m_writer_as_src(m_src_writer), m_writer_max_size(max_size), m_is_reader_memory(true), m_chunk_size(chunk_size), m_last_peak_throughput(0), m_last_chunksize_update_time(0), m_user_param(user_param), m_process_callback(callback)
{
    m_writer_pointer = 0;
}
...
bool filetransfer_node::make_data_packet(NET_Packet& packet)
{
    ...
    if (m_reader)
    {
        size_to_write = (static_cast<u32>(m_reader->elapsed()) >= m_chunk_size) ? m_chunk_size : m_reader->elapsed();
        if (size_to_write)
        {
...
filereceiver_node::filereceiver_node(shared_str const& file_name, receiving_state_callback_t const& callback): m_file_name(file_name), m_is_writer_memory(false), m_process_callback(callback), m_last_read_time(0)
{
    m_writer = FS.w_open(file_name.c_str());
}

filereceiver_node::filereceiver_node(CMemoryWriter* mem_writer, receiving_state_callback_t const& callback): m_is_writer_memory(true), m_process_callback(callback), m_last_read_time(0)
{
