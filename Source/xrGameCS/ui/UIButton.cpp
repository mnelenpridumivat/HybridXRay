...
bool CUIButton::OnMouse(float x, float y, EUIMessages mouse_action)
{
    ...
    if ((WINDOW_LBUTTON_DOWN == mouse_action || WINDOW_LBUTTON_UP == mouse_action || WINDOW_RBUTTON_DOWN == mouse_action || WINDOW_RBUTTON_UP == mouse_action) && HasChildMouseHandler())
        return false;
...
void CUIButton::DrawTexture()
{
    ...
    if (m_bTextureEnable && GetShader() && GetShader()->inited())
    {
        ...
        {
            Frect r = {0, 0, m_UIStaticItem.GetOriginalRect().width(), m_UIStaticItem.GetOriginalRect().height()};
