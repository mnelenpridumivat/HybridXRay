...
void CUICharacterInfo::InitCharacter(u16 id)
{
    ...
    if (m_icons[eName])
    {
        m_icons[eName]->SetTextST(T->m_character_name.c_str());
    }
    if (m_icons[eRank])
    {
        m_icons[eRank]->SetTextST(GetRankAsText(chInfo.Rank().value()));
    }
    if (m_icons[eCommunity])
    {
        m_icons[eCommunity]->SetTextST(chInfo.Community().id().c_str());
    }
    if (m_icons[eReputation])
    {
        m_icons[eReputation]->SetTextST(GetReputationAsText(chInfo.Reputation().value()));
    }
...
{
    if (m_icons[eName])
        m_icons[eName]->SetText("");
    if (m_icons[eRank])
        m_icons[eRank]->SetText("");
    if (m_icons[eCommunity])
        m_icons[eCommunity]->SetText("");
    if (m_icons[eReputation])
        m_icons[eReputation]->SetText("");
    if (m_icons[eRelation])
        m_icons[eRelation]->SetText("");
}
