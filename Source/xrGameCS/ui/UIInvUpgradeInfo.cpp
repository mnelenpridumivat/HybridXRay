...
bool UIInvUpgradeInfo::init_upgrade(Upgrade_type* upgr, CInventoryItem* inv_item)
{
    ...
    if (m_upgrade->is_known())
    {
        ...
        if (upg_res == inventory::upgrade::result_ok || upg_res == inventory::upgrade::result_e_precondition_money || upg_res == inventory::upgrade::result_e_precondition_quest)
        {
        ...
        else
        {
            ...
            switch (upg_res)
            {
                case inventory::upgrade::result_e_unknown:
                    strcpy_s(str_res, sizeof(str_res), "st_upgr_unknown");
                    break;
                case inventory::upgrade::result_e_installed:
                    strcpy_s(str_res, sizeof(str_res), "st_upgr_installed");
                    break;
                case inventory::upgrade::result_e_parents:
                    strcpy_s(str_res, sizeof(str_res), "st_upgr_parents");
                    break;
                case inventory::upgrade::result_e_group:
                    strcpy_s(str_res, sizeof(str_res), "st_upgr_group");
                    break;
