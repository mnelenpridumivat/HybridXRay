    ...
    if (g_pGameLevel)
    {
        ...
        if (ui_game_sp)
        {
            if (!m_pda_section || 0 == xr_strlen(m_pda_section))
                if (ui_game_sp->PdaMenu().IsShown() || ui_game_sp->ActorMenu().IsShown() || ui_game_sp->TalkMenu->IsShown() || ui_game_sp->UIChangeLevelWnd->IsShown())
...
*/
        ...
        else if (!stricmp(m_pda_section, "pda_fraction_war"))
        {
            ui_game_sp->PdaMenu().SetActiveSubdialog("eptFractionWar");
            bShowPda = true;
        }
        else if (!stricmp(m_pda_section, "pda_ranking"))
        {
