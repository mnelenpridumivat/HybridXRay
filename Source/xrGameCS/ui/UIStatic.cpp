...
void CUIStatic::DrawTexture()
{
    if (m_bTextureEnable && GetShader() && GetShader()->inited())
    {
        ...
        {
            Frect r = {0.0f, 0.0f, m_UIStaticItem.GetOriginalRect().width(), m_UIStaticItem.GetOriginalRect().height()};
...
void CUIStatic::TextureClipper(float offset_x, float offset_y, Frect* pClipRect, CUIStaticItem& UIStaticItem)
{
    ...
    else
    {
        Frect r;
        r.x1 = out_rect.left;
        r.x2 = out_rect.right < UIStaticItem.GetOriginalRect().width() ? out_rect.right : UIStaticItem.GetOriginalRect().width();

        r.y1 = out_rect.top;
        r.y2 = out_rect.bottom < UIStaticItem.GetOriginalRect().height() ? out_rect.bottom : UIStaticItem.GetOriginalRect().height();
