...
template<class Element> bool CUIListWnd::AddItem(const char* str, const float shift, void* pData, int value, int insertBeforeIdx)
{
...
template<class Element> bool CUIListWnd::AddItem(Element* pItem, int insertBeforeIdx)
{
    ...
    pItem->InitListItem(Fvector2().set(pItem->GetWndRect().left, m_bVertFlip ? GetHeight() - GetItemsCount() * m_iItemHeight - m_iItemHeight : GetItemsCount() * m_iItemHeight), Fvector2().set(m_iItemWidth, m_iItemHeight));
