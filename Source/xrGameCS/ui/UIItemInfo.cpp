...
void CUIItemInfo::InitItem(CInventoryItem* pInvItem, CInventoryItem* pCompareItem)
{
    ...
    if (UIItemImage)
    {
        ...
        UIItemImage->GetUIStaticItem().SetOriginalRect(float(item_grid_rect.x1 * INV_GRID_WIDTH), float(item_grid_rect.y1 * INV_GRID_HEIGHT), float(item_grid_rect.x2 * INV_GRID_WIDTH), float(item_grid_rect.y2 * INV_GRID_HEIGHT));
        UIItemImage->TextureOn();
        UIItemImage->ClipperOn();
        UIItemImage->SetStretchTexture(true);
        Frect v_r = {0.0f, 0.0f, float(item_grid_rect.x2 * INV_GRID_WIDTH2), float(item_grid_rect.y2 * INV_GRID_HEIGHT2)};
        if (UI()->is_16_9_mode())
            v_r.x2 /= 1.2f;
