...
BOOL CGameObject::net_Spawn(CSE_Abstract* DC)
{
    ...
    else
    {
        ...
        R_ASSERT2(prev_obj == NULL, make_string("previous object ID=%d is %s, hasn't been destroyed but (to destroy = %d)", E->ID, prev_obj->cName().c_str(), prev_obj->getDestroy()).c_str());
    }
