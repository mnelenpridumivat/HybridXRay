...
void CPHSimpleCharacter::PhTune(dReal step)
{
    ...
    if (ph_dbg_draw_mask.test(phDbgCharacterControl))
    {
        if (b_air_contact_state)
            DBG_DrawPoint(cast_fv(dBodyGetPosition(m_body)), m_radius, color_xrgb(255, 0, 0));
    }
#endif
    ...
    if (is_control)
    {
        ...
        if (ph_dbg_draw_mask.test(phDbgCharacterControl))
        {
            ...
            DBG_DrawLine(Fvector().add(cast_fv(dBodyGetPosition(m_body)), dipsp), Fvector().add(Fvector().add(cast_fv(dBodyGetPosition(m_body)), dipsp), Fvector().mul(cast_fv(dBodyGetForce(m_body)), 1.f / 1000.f)), color_xrgb(255, 0, 0));
        }
#endif
        //if(b_clamb_jump){
...
void CPHSimpleCharacter::Collide()
{
    ...
    inherited::Collide();
    if (injuriousMaterialIDX == GAMEMTL_NONE_IDX && (*p_lastMaterialIDX) != GAMEMTL_NONE_IDX && GameMaterialLibrary->GetMaterialByIdx(*p_lastMaterialIDX)->Flags.test(SGameMtl::flInjurious))
