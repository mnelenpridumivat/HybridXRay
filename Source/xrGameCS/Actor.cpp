...
void CActor::g_Physics(Fvector& _accel, float jump, float dt)
{
    ...
    if (Local() && g_Alive())
    {
        ...
        if (!fis_zero(character_physics_support()->movement()->gcontact_HealthLost))
        {
            ...
            {
                SHit       HDS = SHit(character_physics_support()->movement()->gcontact_HealthLost, 0.0f, hdir, di->DamageInitiator(), character_physics_support()->movement()->ContactBone(), di->HitPos(), 0.f, di->HitType());
                //				Hit(&HDS);
