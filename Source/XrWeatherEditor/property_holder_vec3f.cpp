    ...
    refresh_grid_on_change_enum const&  refresh_grid)
{
    m_container->add_property(
        gcnew PropertySpec(
            to_string(identifier), Vec3f::typeid, to_string(category), to_string(description),
            Vec3f(default_value.x, default_value.y, default_value.z), (String ^) nullptr,
            property_converter_vec3f::typeid),
        gcnew property_vec3f(getter, setter));
