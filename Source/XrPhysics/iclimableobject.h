...
public:
    ...
    virtual float DDLowerP(CPHCharacter* actor, Fvector& out_dir) const = 0;   // returns distance and dir to lover
                                                                               // point
    virtual float DDUpperP(CPHCharacter* actor, Fvector& out_dir) const = 0;   // returns distance and dir to upper
                                                                               // point
    //
    // virtual	void			DToAxis				(CPHCharacter	*actor,Fvector &dir)const=0;
    virtual float DDToAxis(CPHCharacter* actor, Fvector& out_dir) const = 0;   // returns distance and dir to ladder
                                                                               // axis
