...
Shader* CResourceManager::_lua_Create(LPCSTR d_shader, LPCSTR s_textures)
{
    ...
    if (Script::bfIsObjectPresent(LSVM, s_shader, "normal_hq", LUA_TFUNCTION))
    {
        ...
        C.iElement = 0;
        //.		C.bDetail			= RESOURCE->_GetDetailTexture(*C.L_textures[0],C.detail_texture,C.detail_scaler);
        // C.bDetail			=
        // RESOURCE->m_textures_description.GetDetailTexture(C.L_textures[0],C.detail_texture,C.detail_scaler);
        C.bDetail  = RESOURCE->m_textures_description.GetDetailTexture(C.L_textures[0], C.detail_texture, C.detail_scaler);
    ...
    else
    {
        if (Script::bfIsObjectPresent(LSVM, s_shader, "normal", LUA_TFUNCTION))
        {
            C.iElement = 0;
            //.			C.bDetail			=
            // RESOURCE->_GetDetailTexture(*C.L_textures[0],C.detail_texture,C.detail_scaler); C.bDetail			=
            // RESOURCE->m_textures_description.GetDetailTexture(C.L_textures[0],C.detail_texture,C.detail_scaler);
            C.bDetail  = RESOURCE->m_textures_description.GetDetailTexture(C.L_textures[0], C.detail_texture, C.detail_scaler);
            S.E[0]     = C._lua_Compile(s_shader, "normal");
        }
...
ShaderElement* CBlender_Compile::_lua_Compile(LPCSTR namesp, LPCSTR name)
{
    ...
    r_End();
    ShaderElement* _r = RESOURCE->_CreateElement(E);
    return _r;
}
