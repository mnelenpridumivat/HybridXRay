...
XRCORE_API AnsiString&
    _ReplaceItems(LPCSTR src, int idx_start, int idx_end, LPCSTR new_items, AnsiString& dst, char separator);
...
template <int count> inline LPSTR
    _GetItem(LPCSTR src, int index, char (&dst)[count], char separator = ',', LPCSTR def = "", bool trim = true)
{
