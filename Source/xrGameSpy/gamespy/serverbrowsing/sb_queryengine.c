...
static void QEStartQuery(SBQueryEngine* engine, SBServer server)
{
    ...
    if (server->state & STATE_PENDINGICMPQUERY)   // send an ICMP ping request
    {
        ...
        memcpy(
            queryBuffer + sizeof(SBICMPHeader), &server->publicip,
        ...
        gsDebugFormat(
            GSIDebugCat_SB, GSIDebugType_Network, GSIDebugLevel_WarmError, "Attempting to send ICMP ping to %s\r\n",
            inet_ntoa(*((struct in_addr*)&server->icmpip)));

        result =
            sndev_set_options(0, SN_DEV_SET_PING_IP, (void*)&optval, sizeof(optval));   // tell SN to ping this addr
    ...
    else   // send a UDP query
    {
        ...
        if (server->publicip == engine->mypublicip && (server->flags & PRIVATE_IP_FLAG))   // try querying the private
                                                                                           // IP
        {
    ...
    SBBool         usequerychallenge)
{
    ...
    server->state &= (unsigned char)~(
        STATE_PENDINGBASICQUERY | STATE_PENDINGFULLQUERY | STATE_PENDINGICMPQUERY | STATE_PENDINGQUERYCHALLENGE |
...
SBServer SBQueryEngineUpdateServerByIP(
    SBQueryEngine* engine,
    const char*    ip,
                 ...
                 server->privateport == saddr.sin_port))   // or has a private, and matches
            {
                if (icmpSocket)
                {
                    if (ParseSingleICMPReply(engine, server, indata, i))
                        break;   // only break if it matches exactly, since we may have multiple outstanding pings to
                                 // the same ICMPIP for different servers!
                }
                else
                {
                    if (engine->queryversion == QVERSION_QR2)
