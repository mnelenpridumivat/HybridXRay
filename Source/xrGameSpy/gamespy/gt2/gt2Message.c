...
static GT2Bool gti2HandleClientResponse(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    if (!gti2ConnectAttemptCallback(
            connection->socket, connection, connection->ip, connection->port, latency, message + GTI2_RESPONSE_LEN,
...
// <=0 on error
gsi_bool _NetworkAdHocSocketRecv(
    int      socket_id,
    char*    buf,
    int      bufferlen,
    int      flags,
...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    gti2BufferWriteData(
        &connection->outgoingBuffer, message + connection->socket->protocolOffset,
