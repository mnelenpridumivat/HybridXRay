...
static void SendReportPacket(NATNegotiator neg)
{
    ...
    gsDebugFormat(
        GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending REPORT to %s:%d (result: %d)\n",
...
static void SendInitPackets(NATNegotiator neg)
{
    ...
    if (p->Packet.Init.usegameport && !neg->initAckRecv[NN_PT_GP])
    {
        ...
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending INIT (GP) to %s:%d...\n",
    ...
    if (!neg->initAckRecv[NN_PT_NN1])
    {
        ...
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending INIT (NN1) to %s:%d...\n",
            inet_ntoa(*(struct in_addr*)&matchup1ip), MATCHUP_PORT1);
    ...
    if (!neg->initAckRecv[NN_PT_NN2])
    {
        ...
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending INIT (NN2) to %s:%d...\n",
    ...
    if (!neg->initAckRecv[NN_PT_NN3])
    {
        ...
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending INIT (NN3) to %s:%d...\n",
...
static void SendPingPacket(NATNegotiator neg)
{
    ...
    gsDebugFormat(
        GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending PING to %s:%d (got remote data: %d)\n",
        inet_ntoa(*(struct in_addr*)&neg->guessedIP), neg->guessedPort, neg->gotRemoteData);
    SendPacket(
        (neg->gameSock != INVALID_SOCKET) ? neg->gameSock : neg->negotiateSock, neg->guessedIP, neg->guessedPort, &p,
        CONNECTPACKET_SIZE);
    ...
    void*                  userdata)
{
...
#if defined(GSI_COMMON_DEBUG)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Negotiate Socket: %d\n",
...
static void NegotiateThink(NATNegotiator neg)
{
    ...
    if (neg->negotiateSock != INVALID_SOCKET)
    {
        ...
        while (CanReceiveOnSocket(neg->negotiateSock))
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "RECV SOCKET ERROR: %d\n",
...
{
    gsDebugFormat(
        GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
    ...
    switch (p->packettype)
    {
        case NN_INITACK:
            ...
            if (p->Packet.Init.porttype > NN_PT_NN3)
                return;   // invalid port
            gsDebugFormat(
                GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Got init ack for port %d\n",
