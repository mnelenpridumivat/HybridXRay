...
GHIEncryptionResult ghiEncryptorSslDecryptRecv(
    struct GHIConnection* connection,
    struct GHIEncryptor* theEncryptor,
    char* theDecryptedBuffer,
    int* theDecryptedLength)
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
    ...
    MD5Update(
        &sslInterface->finishHashMD5, (unsigned char*)&helloMsg + sizeof(gsSSLRecordHeaderMsg),
        sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg));
    SHA1Input(
        &sslInterface->finishHashSHA1, (unsigned char*)&helloMsg + sizeof(gsSSLRecordHeaderMsg),
        sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg));
    ...
    int* theEncryptedLength)
{
    if (theEncryptor != NULL)
    {
        ...
        else
        {
            ...
            RC4Encrypt(
                &sslInterface->sendRC4, (const unsigned char*)thePlainTextBuffer,
    ...
    while (readPos < *theEncryptedLength)
    {
        ...
        switch (header->contentType)
        {
            case GS_SSL_CONTENT_HANDSHAKE: {
                ...
                if (connection->encryptor.mSessionEstablished)
                    RC4Encrypt(
                        &sslInterface->recvRC4, (const unsigned char*)&theEncryptedBuffer[readPos],
            ...
            case GS_SSL_CONTENT_APPLICATIONDATA: {
                ...
                memcpy(
                    theDecryptedBuffer + writePos, &theEncryptedBuffer[readPos],
            ...
            case GS_SSL_CONTENT_CHANGECIPHERSPEC:
                ...
                gsDebugFormat(
                    GSIDebugCat_HTTP, GSIDebugType_Network, GSIDebugLevel_Notice,
            ...
            case GS_SSL_CONTENT_ALERT:
                ...
                gsDebugFormat(
                    GSIDebugCat_HTTP, GSIDebugType_Network, GSIDebugLevel_WarmError,
    ...
    ghiEncryptorProcessSSLHandshake(struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data)
{
    ...
    while (data->pos < data->len)
    {
            ...
            while (data->pos < certListEndPos)
            {
                ...
                if (certCount == 1)
                {
                    ...
                    sslInterface->serverpub.modulus.mLength =
                        (unsigned int)((temp - 1) / GS_LARGEINT_DIGIT_SIZE_BYTES);   //-1 to subtract the previous 0x00
                                                                                     //byte
        ...
        else if (messageType == GS_SSL_HANDSHAKE_SERVERHELLODONE)
        {
            ...
            // add it to the running handshake hash
            MD5Update(
                &sslInterface->finishHashMD5, (unsigned char*)&data->data[messageStart],
                (unsigned int)(data->pos - messageStart));
            SHA1Input(
                &sslInterface->finishHashSHA1, (unsigned char*)&data->data[messageStart],
                (unsigned int)(data->pos - messageStart));

            // Make sure there is room in the send buffer for the response messages
            tempInt =
                (int)(sizeof(gsSSLClientKeyExchangeMsg) + sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES);
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
            ...
            gsCryptRSAEncryptBuffer(
                &sslInterface->serverpub, sslInterface->premastersecret, GS_SSL_MASTERSECRET_LEN,
            ...
            // add it to the running handshake hash
            MD5Update(
                &sslInterface->finishHashMD5, (unsigned char*)clientKeyExchange + sizeof(gsSSLRecordHeaderMsg),
            ...
            SHA1Input(
                &sslInterface->finishHashSHA1, (unsigned char*)clientKeyExchange + sizeof(gsSSLRecordHeaderMsg),
                sizeof(gsSSLClientKeyExchangeMsg) - sizeof(gsSSLRecordHeaderMsg) +
                    sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES);
            ...
            MD5Init(&sslInterface->finishHashMD5);
            MD5Update(
                &sslInterface->finishHashMD5, (unsigned char*)sslInterface->clientWriteMACSecret,
            ...
            RC4Encrypt(
                &sslInterface->sendRC4, ((unsigned char*)finalHandshake) + sizeof(gsSSLRecordHeaderMsg),
