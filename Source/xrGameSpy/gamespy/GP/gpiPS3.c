...
GPResult gpiCheckNpStatus(GPConnection* connection)
{
    ...
    gsDebugFormat(
        GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment, "gpiCheckNpStatus: sceNpGetStatus - status = %d\n",
        status);
    ...
    if (status == SCE_NP_MANAGER_STATUS_ONLINE)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment,
...
GPResult gpiSyncNpBuddies(GPConnection* connection)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        gpiProfileSearchUniquenick(
            connection, npId.handle.data, &iconnection->namespaceID, 1, GP_NON_BLOCKING,
    ...
    if (arg->result == GP_NO_ERROR)
    {
        ...
        if (_tcslen(arg->uniquenick) != 0)
        {
            ...
            else
            {
                ...
                /////////////////////////////////////////////////////////////////////////
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_WarmError,
