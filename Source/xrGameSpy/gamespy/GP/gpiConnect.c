...
static GPResult gpiStartConnect(GPConnection* connection, GPIOperation* operation)
{
    ...
    if (!gsUdpEngineIsInitialized())
    {
        ...
        if (anError != GS_UDP_NO_ERROR)
        {
            ...
            {
                gsDebugFormat(
                    GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_Comment, "Port %d failed, trying next port\n",
                    peerPort);
                anError = gsUdpEngineInitialize(++peerPort, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
            }
            if (anError != GS_UDP_NO_ERROR)
    ...
    anError = gsUdpEngineAddMsgHandler(
        iconnection->mHeader, iconnection->mHeader, NULL, gpiPeerAcceptedCallback, gpiPeerLeftCallback,
...
static GPResult gpiSendLogin(GPConnection* connection, GPIConnectData* data)
{
    ...
    //////////////////////////
    sprintf(
        buffer, "%s%s%s%s%s%s", data->passwordHash, "                                                ", user,
