    ...
    int                   gameMapNameLen)
{
    ...
    aResult = gpSetStatusInfoA(
        connection, statusState, hostIp, hostPrivateIp, queryPort, hostPort, sessionFlags, richStatus_A,
        (int)strlen(richStatus_A), gameType_A, (int)strlen(gameType_A), gameVariant_A, (int)strlen(gameVariant_A),
        gameMapName_A, (int)strlen(gameMapName_A));
...
GPResult gpGetBuddyStatusInfoKeys(GPConnection* connection, int index, GPCallback callback, void* userData)
{
    ...
    if (pProfile->buddyStatus)
        CallbackError(
            connection, GP_PARAMETER_ERROR, GP_BM_EXT_INFO_NOT_SUPPORTED,
            ...
            if (!pProfile->buddyStatusInfo && !pProfile->buddyStatus) CallbackError(
                ...
                if (pProfile->buddyStatusInfo && pProfile->buddyStatusInfo->statusState == GP_OFFLINE) CallbackError(
