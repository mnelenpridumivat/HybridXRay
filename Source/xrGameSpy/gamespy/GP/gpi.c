...
GPResult gpiProcessConnectionManager(GPConnection* connection)
{
    ...
    do
    {
        ...
        while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
        {
            ...
            gsDebugFormat(
                GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "CMD: %s\n",
            ...
            if (str != NULL)
            {
                ...
                {
                    gsDebugFormat(
                        GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_HotError,
...
GPResult gpiProcess(GPConnection* connection, int blockingOperationID)
{
    ...
    assert(
        (iconnection->connectState == GPI_NOT_CONNECTED) || (iconnection->connectState == GPI_CONNECTING) ||
...
void gpiReport(GPConnection* connection, void (*report)(const char* output))
{
    ...
    if (nProfiles)
    {
        ...
        report(buf);
        sprintf(
            buf, "BuddyStatus: %d (%d%%) %d bytes (%d avg)", nBuddyStatus, nBuddyStatus * 100 / nProfiles, nBuddyMemory,
            nBuddyMemory / max(nBuddyStatus, 1));
        report(buf);
        sprintf(
            buf, "InfoCache: %d (%d%%) %d bytes (%d avg)", nInfoCache, nInfoCache * 100 / nProfiles, nInfoMemory,
