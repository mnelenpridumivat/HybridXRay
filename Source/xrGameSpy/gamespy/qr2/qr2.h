...
extern "C"
{
    ...
    ***************/
    typedef enum
    {
        ...
        e_qrnochallengeerror,   // no challenge was received from the master - either the master is down, or a firewall
                                // is blocking UDP
        qr2_error_t_count
    } qr2_error_t;
...
#define QR_MAGIC_2       0xFD
    ...
    ********/
    ...
    [keytype] should be used to determine whether the player or team count is being requested (key_player or key_team
    ...
    ********/
    ...
    server. [error] is a code that can be used to determine the specific listing error. [errmsg] is a human-readable
    error string returned from the master server. [userdata] is the pointer that was passed into qr2_init. You can use
    ...
    ********/
    ...
    [key] is the string name of the key. Player keys should end in "_" (such as "score_") and team keys should end in
    ...
    *******************/
    ...
    [natnegotiate] is 1 if the server supports GameSpy's NAT-negotiation technology (or another similar technology)
         ...
         pass 0 to prevent the server from being listed if it is behind a NAT that cannot be traversed by outside
    clients. [qr2_*_callback] are your callback functions, these cannot be NULL [userdata] is an optional,
    implementation specific parameter that will be passed to all callback functions. Use it to store an object or
    structure pointer if needed.
