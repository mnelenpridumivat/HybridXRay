...
/*#elif defined(_PS2)
    ...
    static void* gsiResolveHostnameThread(void* arg)
    {
        ...
        if (aHostAddr != 0)
        {
            ...
            gsDebugFormat(
                GSIDebugCat_HTTP, GSIDebugType_State, GSIDebugLevel_Comment, "Resolved host '%s' to ip '%s'\n",
                handle->hostname, ip);
...
#define _LEAP_YEAR_ADJUST 17L
        ...
        int sc)
    {
        ...
        tmptim = ((((tmptim)*365L + ((long)(yr - 1) >> 2) - (long)_LEAP_YEAR_ADJUST + (long)tmpdays) * 24L + (long)hr) *
                      60L +
                  (long)mn) *
                60L +
            (long)sc;
    ...
    time_t time(time_t* timer)
    {
        ...
        tim = _gmtotime_t(
            DEC(clocktime.year) + 2000, DEC(clocktime.month), DEC(clocktime.day), DEC(clocktime.hour),
...
///////////////////////////////////////////////////////////////////////////////
    ...
    static const char* GOAGetUniqueID_Internal(void)
    {
        ...
        // format it
        sprintf(
            keyval, "%02X%02X%02X%02X%02X%02X0000", MAC[0] & 0xFF, MAC[1] & 0xFF, MAC[2] & 0xFF, MAC[3] & 0xFF,
