...
static gsi_i32 gsCryptRSAOAEPEncryptBuffer(
    const gsCryptRSAKey* publicKey,
    const unsigned char* plainText,
    gsi_u32              len,
    unsigned char        buffer[GS_CRYPT_RSA_BYTE_SIZE]);
static gsi_i32 gsCryptRSAOAEPDecryptBuffer(
...
#else
static gsi_i32 gsCryptRSAPKCS1EncryptBuffer(
    const gsCryptRSAKey* publicKey,
    const unsigned char* plainText,
    gsi_u32              len,
    unsigned char        buffer[GS_CRYPT_RSA_BYTE_SIZE]);
static gsi_i32 gsCryptRSAPKCS1DecryptBuffer(
    const gsCryptRSAKey* privateKey,
...
gsi_i32 gsCryptRSAEncryptBuffer(
    const gsCryptRSAKey* publicKey,
    const unsigned char* plainText,
    gsi_u32              len,
    unsigned char        buffer[GS_CRYPT_RSA_BYTE_SIZE])
...
#ifdef GS_CRYPT_RSA_ES_OAEP
gsi_i32 gsCryptRSAOAEPEncryptBuffer(
    const gsCryptRSAKey* publicKey,
    const unsigned char* plainText,
    gsi_u32              len,
    unsigned char        buffer[GS_CRYPT_RSA_BYTE_SIZE])
...
#if (GS_CRYPT_HASHSIZE == GS_CRYPT_MD5_HASHSIZE)
    const gsi_u8 lhash[GS_CRYPT_HASHSIZE] = {0xd4, 0x1d, 0xd4, 0x1d, 0x8c, 0xd9, 0x8f, 0x00, 0xb2,
                                             0x04, 0xe9, 0x80, 0x09, 0x98, 0xec, 0xf8, 0x42, 0x7e};   // hash of ""
#else
    const gsi_u8 lhash[GS_CRYPT_HASHSIZE] = {0xda, 0x39, 0xa3, 0xee, 0x5e, 0x6b, 0x4b, 0x0d, 0x32, 0x55, 0xbf,
                                             0xef, 0x95, 0x60, 0x18, 0x90, 0xaf, 0xd8, 0x07, 0x09};   // hash of ""
#endif
...
#else
gsi_i32 gsCryptRSAPKCS1EncryptBuffer(
    const gsCryptRSAKey* publicKey,
    const unsigned char* plainText,
    gsi_u32              len,
    unsigned char        buffer[GS_CRYPT_RSA_BYTE_SIZE])
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_i32 gsCryptRSADecryptBuffer(
    const gsCryptRSAKey* privateKey,
    const unsigned char  cipherText[GS_CRYPT_RSA_BYTE_SIZE],
    unsigned char*       plainText,
    gsi_u32*             lenout)
    ...
    gsi_u32*             lenOut)
{
    ...
    gsi_u8 md5Header[18]  = {0x30, 0x20, 0x30, 0x0C, 0x06, 0x08, 0x2A, 0x86, 0x48,
...
gsi_i32 gsCryptRSAVerifySignedHash(
    const gsCryptRSAKey* publicKey,
    const unsigned char* hash,
    gsi_u32              hashLen,
    ...
    gsi_u32              sigLen)
{
    ...
    if ((i + sizeof(md5Header) + GS_CRYPT_MD5_HASHSIZE) == (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES))
    {
        ...
        (i + sizeof(sha1Header) + GS_CRYPT_SHA1_HASHSIZE) == (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES))
    {
        // SHA1 Hash
        //   1. verify header
