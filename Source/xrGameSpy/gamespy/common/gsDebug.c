...
char* gGSIDebugCatStrings[GSIDebugCat_Count]     = {" APP", " GP ", "PEER", " QR2", "  SB", "  V2",
                                                    "  AD", "  NN", "HTTP", "CDKY", " CMN"};
char* gGSIDebugTypeStrings[GSIDebugType_Count]   = {" NET", "FILE", " MEM", "STAT", "MISC"};
char* gGSIDebugLevelStrings[GSIDebugLevel_Count] = {"*ERR", "****", "----", "    ", "    ", "    ", "  ->"};
char* gGSIDebugLevelDescriptionStrings[8]        = {"None",      "<None+1>",    "<Normal>",    "<Debug>",
                                                    "<Verbose>", "<Verbose+1>", "<Verbose+2>", "<Hardcore>"};
    ...
    va_list          theParamList)
{
    ...
    if (aCurLevel & theLevel)   // check the flag
    {
        ...
        {
            fprintf(
                gGSIDebugInstance.mGSIDebugFile, "[%s][%s][%s] ", gGSIDebugCatStrings[theCat],
...
void gsSetDebugLevel(GSIDebugCategory theCat, GSIDebugType theType, GSIDebugLevel theLevel)
{
    ...
    if (theCat == GSIDebugCat_Count)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Misc, GSIDebugLevel_Debug,
    ...
    if (theType == GSIDebugType_Count)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Misc, GSIDebugLevel_Debug, "Debug level set to %s for all types\r\n",
            gGSIDebugLevelDescriptionStrings[gsiDebugLog2(theLevel)]);
        return;
    }
    ...
    if (gGSIDebugInstance.mGSIDebugLevel[theCat][theType] != theLevel)
    {
        ...
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Misc, GSIDebugLevel_Comment, "Changing debug level: [%s][%s][%02X]\r\n",
            gGSIDebugCatStrings[theCat], gGSIDebugTypeStrings[theType], theLevel);
