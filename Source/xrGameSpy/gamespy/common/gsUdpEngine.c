...
void gsUdpClosedRoutingCB(GT2Connection theConnection, GT2CloseReason reason)
{
    ...
    gsDebugFormat(
        GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Connection closed to %s\n",
    ...
    for (aIndex = 0; aIndex < len; aIndex++)
    {
        ...
        if (aRemotePeerIdx != NOT_FOUND)
        {
            if (aTempHandler->mConnected)
            {
                ...
                aTempHandler->mConnected(
                    gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), aCode,
...
void gsUdpPingRoutingCB(GT2Connection theConnection, int theLatency)
{
    ...
    gsDebugFormat(
        GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Received ping from %s\n",
        gt2AddressToString(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), anAddr));
    len = ArrayLength(aUdp->mMsgHandlers);
    for (index = 0; index < len; index++)
    {
        ...
        if (aHandler->mPingReply)
        {
            ...
            aHandler->mPingReply(
                gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), (unsigned int)theLatency,
                aHandler->mUserData);
            return;
        }
    }
...
void gsUdpReceivedRoutingCB(GT2Connection theConnection, GT2Byte* theMessage, int theMessageLen, GT2Bool reliable)
{
    ...
    gsDebugFormat(
        GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Received data from %s\n",
        gt2AddressToString(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), anAddr));
        ...
        if (index != NOT_FOUND)
        {
            ...
            if (aHandlerFound->mReceived)
            {
                gsDebugFormat(
                    GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment,
                    "[Udp Engine] Passed to message handler\n");
                aHandlerFound->mReceived(
                    gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), theMessage + GS_UDP_MSG_HEADER_LEN,
                    (unsigned int)(theMessageLen - GS_UDP_MSG_HEADER_LEN), reliable, aHandlerFound->mUserData);
                return;
            }
        }
    }
    ...
    if (aUdp->mAppRecvData)
    {
        gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Passed to app\n");
        aUdp->mAppRecvData(
            gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), theMessage, (unsigned int)theMessageLen,
            reliable, aUdp->mAppUserData);
    }
    GSI_UNUSED(anAddr);
}
    ...
    int            len)
{
    ...
    gsDebugFormat(
        GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Connection attempt from %s\n",
...
// functions below.
GSUdpErrorCode gsUdpEngineInitialize(
    unsigned short                 thePort,
    ...
    void*                          theAppUserData)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_WarmError,
    ...
    int            timeOut)
{
    ...
    {
        gsDebugFormat(
            GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Debug,
    ...
    else
    {
        ...
        gt2Connect(
            aUdp->mSocket, &aRemotePeer.mConnection, anAddr, (unsigned char*)theInitMsg, GS_UDP_MSG_HEADER_LEN, timeOut,
            &aCallbacks, GT2False);
...
// freeSpace = 1460 - 16 - 7
GSUdpErrorCode gsUdpEngineSendMessage(
    unsigned int   theIp,
    unsigned short thePort,
...
GSUdpErrorCode gsUdpEngineAddMsgHandler(
    char                          theInitMsg[GS_UDP_MSG_HEADER_LEN],
    char                          theHeader[GS_UDP_MSG_HEADER_LEN],
