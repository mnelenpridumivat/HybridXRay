    ...
    if (theResult == GHTTPSuccess)
    {
        ...
        else
        {
            ...
            else if (response.mResponseCode != WSLogin_Success)
            {
                ...
                gsi_is_false(gsXmlMoveToChild(theResponseXml, "certificate")) ||
            ...
            else
            {
                ...
                memcpy(
                    &response.mPrivateData.mPeerPrivateKey.modulus, &cert->mPeerPublicKey.modulus,
    ...
    void*           userData)
{
    ...
    if (writer != NULL)
    {
        ...
        aTask = gsiExecuteSoap(
            wsAuthServiceURL, WS_AUTHSERVICE_LOGINREMOTEAUTH_SOAP, writer, wsLoginRemoteAuthCallback,
            (void*)requestData);
        if (aTask == NULL)
        {
            gsXmlFreeWriter(writer);
