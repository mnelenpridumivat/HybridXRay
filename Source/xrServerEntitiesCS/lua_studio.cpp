...
void engine::fill_class_data(cs::lua_debugger::backend& backend, cs::lua_debugger::value_to_expand& value_to_expand, lua_State* const state)
{
...

    {
        ...
        for (; i != e; ++i)
            value_to_expand.add_value((*i).base->name(), class_name(type, sizeof(type), *(*i).base), "{...}", cs::lua_debugger::icon_type_class_base);
    }
...
void engine::expand_class(cs::lua_debugger::backend& backend, cs::lua_debugger::value_to_expand& value, lua_State* const state)
{
...
void engine::expand_class_instance(cs::lua_debugger::backend& backend, cs::lua_debugger::value_to_expand& value_to_expand, lua_State* const state)
{
    ...
    if (object->crep())
    {
        ...
        string4096                  value;
        backend.value_to_string(value, sizeof(value), state, -1, icon_type, true);
        value_to_expand.add_value(class_rep->name(), type, value, cs::lua_debugger::icon_type_class);
    }
...
void engine::expand_user_data(cs::lua_debugger::backend& backend, cs::lua_debugger::value_to_expand& value, lua_State* const state)
{
    luabind::detail::object_rep* object = luabind::detail::is_class_object(state, -1);
    if (object)
    {
...
bool engine::expand_value(cs::lua_debugger::backend& backend, cs::lua_debugger::value_to_expand& value, lua_State* const state)
{
    switch (lua_type(state, -1))
    {
        case engine::lua_type_nil:
