...
template<typename _client_type, typename _server_type> IC void CObjectFactory::add(const CLASS_ID& clsid, LPCSTR script_clsid)
{
    {
...
template<typename _unknown_type> IC void CObjectFactory::add(const CLASS_ID& clsid, LPCSTR script_clsid)
{
    ...
    static_assert(sizeof(CObjectItemSingle<_unknown_type, object_type_traits::is_base_and_derived<CLIENT_BASE_CLASS, _unknown_type>::value>) == sizeof(CObjectItemAbstract), "");
    auto Result = xr_new<CObjectItemSingle<_unknown_type, object_type_traits::is_base_and_derived<CLIENT_BASE_CLASS, _unknown_type>::value>>(clsid, script_clsid);
    add(Result
