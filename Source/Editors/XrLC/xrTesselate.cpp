...
Vertex* tessEdge(Face* F, int id1, int id2)
{
    ...
    // project
    p1.build(V1->P, V1->N);
    p1.project(pr2, V2->P);   // projected point 2 on plane 1
    p2.build(V2->P, V2->N);
    p2.project(pr1, V1->P);   // projected point 1 on plane 2
...
void Face::TessMark(int E)
{
    ...
    if ((TESS_MASK & TESS_EDGE_0) && (TESS_MASK & TESS_EDGE_1) && !(TESS_MASK & TESS_EDGE_2))
        TessMark(2);
    if ((TESS_MASK & TESS_EDGE_0) && (TESS_MASK & TESS_EDGE_2) && !(TESS_MASK & TESS_EDGE_1))
        TessMark(1);
    if ((TESS_MASK & TESS_EDGE_1) && (TESS_MASK & TESS_EDGE_2) && !(TESS_MASK & TESS_EDGE_0))
        TessMark(0);
}
