...
void CBuild::xrPhase_TangentBasis()
{
    ...
    */
    ...
    xr_vector<float>& v_position = input[0].floatVector_;
    v_position.reserve(v_count_reserve);
    input[1].Name_             = "normal";
    xr_vector<float>& v_normal = input[1].floatVector_;
    v_normal.reserve(v_count_reserve);
    input[2].Name_         = "tex0";
    xr_vector<float>& v_tc = input[2].floatVector_;
    v_tc.reserve(v_count_reserve);
    input[3].Name_            = "indices";
    xr_vector<int>& v_indices = input[3].intVector_;
    v_indices.reserve(i_count_reserve);
    ...
    // ************************************* Bind declarators
    // bind
    xr_vector<float>& o_tc = output[0].floatVector_;
    R_ASSERT(output[0].Name_ == "tex0");
    xr_vector<float>& o_tangent = output[1].floatVector_;
    R_ASSERT(output[1].Name_ == "tangent");
    xr_vector<float>& o_binormal = output[2].floatVector_;
    R_ASSERT(output[2].Name_ == "binormal");
    xr_vector<int>& o_indices = output[3].intVector_;
    R_ASSERT(output[3].Name_ == "indices");
