...
u32 PlaceData(xr_vector<vecW>& C, vecW& P)
{
    ...
    for (xr_vector<vecW>::iterator it = C.begin(); it != C.end(); it++)
    {
        ...
        if (0 != memcmp(it->begin(), P.begin(), S * sizeof(u16)))
        {
            pos += S + 1;
            continue;
        }
...
void CalculateRelSet(Fvector& pos, vecW& rel_set)
{
    ...
    minX = iROUND(floorf(Volume.min.x));
    clamp(minX, 0, int(g_pvs_X) - 1);
    minY = iROUND(floorf(Volume.min.y));
    clamp(minY, 0, int(g_pvs_Y) - 1);
    minZ = iROUND(floorf(Volume.min.z));
    clamp(minZ, 0, int(g_pvs_Z) - 1);

    maxX = iROUND(ceilf(Volume.max.x));
    clamp(maxX, 0, int(g_pvs_X) - 1);
    maxY = iROUND(ceilf(Volume.max.y));
    clamp(maxY, 0, int(g_pvs_Y) - 1);
    maxZ = iROUND(ceilf(Volume.max.z));
    clamp(maxZ, 0, int(g_pvs_Z) - 1);
        ...
        for (int x = 0; x < nx; x++)
        {
            for (int y = 0; y < ny; y++)
            {
                ...
                for (i = 0; i < occluders.size(); i++)
                {
                    ...
                    P.mul(.5f);
                    T = P.distance_to(occluders[i].V1);
                    if (T > R)
                        R = T;
                    T = P.distance_to(occluders[i].V2);
                    if (T > R)
                        R = T;
                    T = P.distance_to(occluders[i].V4);
                    if (T > R)
                        R = T;
