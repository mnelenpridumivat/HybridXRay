...
void  InternalRender()
{
    ...
    for (u32 i = 0; i < g_nodes.size(); i++)
    {
        ...
        int nx = iFloor(P.x / g_params.fPatchSize + 0.5f);
        clamp(nx, 0, dimX - 1);
        int nz = iFloor(P.z / g_params.fPatchSize + 0.5f);
        clamp(nz, 0, dimZ - 1);
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case ID_VIEW_NDEPTH:
                    ShowDepth(hWnd);
                    break;
                case ID_VIEW_NHEIGHT:
                    ShowHeight(hWnd);
                    break;
                case ID_VIEW_NSECTOR:
                    ShowSectors(hWnd);
                    break;
                case ID_VIEW_NNORMALS:
                    ShowNormals(hWnd);
                    break;
                case ID_VIEW_NLIGHT:
                    ShowLight(hWnd);
                    break;
                case ID_VIEW_NSUBDIV:
                    ShowSubdiv(hWnd);
                    break;
                case ID_VIEW_COVER1:
                    ShowCover(hWnd, 0);
