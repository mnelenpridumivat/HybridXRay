...
BOOL ValidNode(vertex& N)
{
    ...
    Fvector PointUp;
    PointUp.set(N.Pos);
    PointUp.y += RCAST_Depth / 2;
    Fvector PointDown;
    PointDown.set(N.Pos);
    PointDown.y -= RCAST_Depth / 2;
...
{
    Nodes smoothed;
    smoothed.reserve(g_nodes.size());
    Marks mark;
    mark.assign(g_nodes.size(), false);
    ...
    for (u32 i = 0; i < g_nodes.size(); i++)
    {
        ...
        vNorm.set(N.Plane.n);
        vOffs.set(N.Pos);
        Mgc::OrthogonalPlaneFit(4, (Mgc::Vector3*)data, *((Mgc::Vector3*)&vOffs), *((Mgc::Vector3*)&vNorm));
        if (vNorm.y < 0)
            vNorm.invert();
    ...
    if (inv_count)
        Msg("%d invalid nodes detected", inv_count);
}
