...
void CExportSkeleton::SSplit::CalculateTB()
{
    ...
    int i_id=-1;
    input[++i_id].Name_= "position";	xr_vector<float>&	i_position	= input[i_id].floatVector_; i_position.reserve
(v_count_reserve); input[++i_id].Name_= "normal";		xr_vector<float>&	i_normal	= input[i_id].floatVector_;
i_normal.reserve	(v_count_reserve);
    input[++i_id].Name_= "tex0";		xr_vector<float>&	i_tc		= input[i_id].floatVector_;	i_tc.reserve
(v_count_reserve); input[++i_id].Name_= "c_w0_b0";  	xr_vector<float>&	i_c_w0_b0	= input[i_id].floatVector_;
i_c_w0_b0.reserve	(v_count_reserve); input[++i_id].Name_= "w1_w2_w3";	xr_vector<float>&	i_w1_w2_w3	=
input[i_id].floatVector_;	i_w1_w2_w3.reserve	(v_count_reserve); input[++i_id].Name_= "b1_b2_b3";	xr_vector<float>&
i_b1_b2_b3	= input[i_id].floatVector_;	i_b1_b2_b3.reserve	(v_count_reserve); input[++i_id].Name_= "indices";
xr_vector<int>&		i_indices	= input[i_id].intVector_;	i_indices.reserve	(i_count_reserve);
    ...
    for (SkelVertIt vert_it=m_Verts.begin(); vert_it!=m_Verts.end(); vert_it++){
        ...
        i_position.push_back(iV.offs.x);	    	    i_position.push_back(iV.offs.y);
i_position.push_back(iV.offs.z); i_normal.push_back	(iV.norm.x);  	    	    i_normal.push_back	(iV.norm.y);
i_normal.push_back	(iV.norm.z); i_tc.push_back		(iV.uv.x);	    		    i_tc.push_back		(iV.uv.y);
i_tc.push_back		(0); u32 sz				= iV.bones.size(); i_c_w0_b0.push_back	(*(float*)&sz);
i_c_w0_b0.push_back(iV.bones[0].w);   			i_c_w0_b0.push_back	(*(float*)&iV.bones[0].id); u32 pt_w			=
i_w1_w2_w3.size(); u32 pt_b			= i_b1_b2_b3.size(); i_w1_w2_w3.push_back(0.f);	i_w1_w2_w3.push_back(0.f);
i_w1_w2_w3.push_back(0.f); i_b1_b2_b3.push_back(0.f);	i_b1_b2_b3.push_back(0.f);	i_b1_b2_b3.push_back(0.f); if
(iV.bones.size()>1){ i_w1_w2_w3[pt_w+0]			= iV.bones[1].w; i_b1_b2_b3[pt_b+0]			=
(*(float*)&iV.bones[1].id); if (iV.bones.size()>2){ i_w1_w2_w3[pt_w+1] 		= iV.bones[2].w;
...
void CExportSkeleton::SSplit::CalculateTB()
{
    ...
    retrive_data_from_mender_otput( m_Verts, m_Faces, mender_in_out_verts, mender_in_out_indices,
    mender_mapping_out_to_in_vert  );
