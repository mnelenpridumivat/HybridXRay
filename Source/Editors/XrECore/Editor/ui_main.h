...
public:
    ...
    void Quit()
    {
        m_Flags.set(flNeedQuit, TRUE);
    }

    u32& GetRenderWidth()
    {
        return EDevice->dwWidth;
    }
    u32& GetRenderHeight()
    {
        return EDevice->dwHeight;
    }
    int GetRealWidth()
    {
        return EDevice->dwRealWidth;
    }
    int GetRealHeight()
    {
        return EDevice->dwRealHeight;
    }

    IC float ZFar()
    {
        return EDevice->m_Camera.m_Zfar;
    }
    IC TShiftState GetShiftState()
    {
        return m_ShiftState;
    }
    ...
    void SetRenderQuality(float q)
    {
        EDevice->m_ScreenQuality = q;
    }
    // mouse action
    void EnableSelectionRect(bool flag);
    void UpdateSelectionRect(const Ivector2& from, const Ivector2& to);

    void MouseMultiClickCapture(bool b)
    {
        m_MouseMultiClickCaptured = b;
    }

    bool IsMouseCaptured()
    {
        return m_MouseCaptured | m_MouseMultiClickCaptured;
    }
    bool IsMouseInUse()
    {
        return bMouseInUse;
    ...
    EEditorState GetEState()
    {
        return m_EditorState.back();
    }
    bool ContainEState(EEditorState st)
    {
        return std::find(m_EditorState.begin(), m_EditorState.end(), st) != m_EditorState.end();
    }
...
public:
    ...
    SPBItem*     ProgressLast()
    {
        return m_ProgressItems.empty() ? 0 : m_ProgressItems.back();
    }
