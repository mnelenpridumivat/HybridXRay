...
bool item_area_sort_pred(const SSimpleImage& item0, const SSimpleImage& item1)
{
    return ((item0.Area() > item1.Area()) || (item0.LongestEdge() > item1.LongestEdge()));
}
extern bool Stbi_Load(LPCSTR full_name, U32Vec& data, u32& w, u32& h, u32& a);

int CImageManager::CreateMergedTexture(
    u32              layer_cnt,
    SSimpleImageVec& src_images,
    SSimpleImage&    dst_image,
    int              dest_width,
    int              dest_height,
    Fvector2Vec&     dest_offset,
    Fvector2Vec&     dest_scale,
    boolVec&         dest_rotate,
    U32Vec&          dest_remap)
{
    if (src_images.empty())
    ...
    for (auto s_it = src_images.begin(); s_it != src_images.end(); s_it++)
    {
        ...
        for (u32 k = 0; k < layer_cnt; k++)
        {
            if (!bRotated)
                blit(
                    dest_layers[k].data(), dest_width, dest_height, s_it->layers[k].data(), s_it->w, s_it->h, R.lt.x,
                    R.lt.y);
            else
                blit_r(
                    dest_layers[k].data(), dest_width, dest_height, s_it->layers[k].data(), s_it->w, s_it->h, R.lt.x,
    ...
    U32Vec&                  dest_remap)
{
    ...
    for (s_it = src_items.begin(); s_it != src_items.end(); s_it++)
    {
        ...
        if (!bRotated)
            blit(
                dest_pixels.data(), dest_width, dest_height, s_it->layers.back().data(), s_it->w, s_it->h, R.lt.x,
                R.lt.y);
        else
            blit_r(
                dest_pixels.data(), dest_width, dest_height, s_it->layers.back().data(), s_it->w, s_it->h, R.lt.x,
