...
{
    m_VNormalsRefs++;
    if (m_VertexNormals || (m_Normals && m_Parent->m_objectFlags.is(CEditableObject::eoNormals)))
        return;
...
%3.2f]",m_Parent->GetName(),VPUSH(m_Vertices[m_Faces[f_i].pv[k].pindex])); #ifdef 0 Fvector p0; p0 =
m_Vertices[m_Faces[f_i].pv[k].pindex]; Tools->m_DebugDraw.AppendPoint(p0, 0xffff0000, true, true, "invalid vNORMAL");
#endif
                        N.set	(m_FaceNormals[a_lst.front()]);
                    }
                }else
