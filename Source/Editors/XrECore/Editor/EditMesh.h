...
public:
    ...
    IC float* getVMdata(int start)
    {
        return &*(vm.begin() + start * dim);
    }
    IC int VMdatasize()
    {
        return vm.size() * sizeof(float);
    }
    IC int* getVIdata()
    {
        return &*vindices.begin();
    }
    IC int VIdatasize()
    {
        return vindices.size() * sizeof(int);
    }
    IC int* getPIdata()
    {
        return &*pindices.begin();
    }
    IC int PIdatasize()
    {
        return pindices.size() * sizeof(int);
    }
    IC int size()
    {
        return vm.size() / dim;
    ...
    IC void appendUV(const Fvector2& uv)
    {
        appendUV(uv.x, uv.y);
    }
    IC void appendW(const float w)
    {
        vm.push_back(w);
    }
    IC void appendVI(const int vi)
    {
...
struct ECORE_API st_SVert
{
    ...
    public:
        ...
        bool similar(const bone& b)
        {
            return (id == b.id) && fsimilar(w, b.w, EPS_L);
        }
    };
...
public:
    void sort_by_weight()
    {
        std::sort(bones.begin(), bones.end(), compare_by_weight);
    }
    void sort_by_bone()
    {
        std::sort(bones.begin(), bones.end(), compare_by_bone);
    }
};
// faces
struct st_Face;
struct ECORE_API st_FaceVert
{
    ...
    bool            gt(const st_FaceVert& v) const
    {
        return pindex > v.pindex;
    }
    bool eq(const st_FaceVert& v) const
    {
        return pindex == v.pindex;
    }
};
...
public:
    ...
    void        GenerateVNormals(bool force = false);
...
public:
    ...
    IC BOOL   Visible()
    {
        return m_Flags.is(flVisible);
    }
    IC void Show(BOOL bVisible)
    {
        m_Flags.set(flVisible, bVisible);
    }
    ...
    IC const st_Face* GetFaces()
    {
        return m_Faces;
    }
    IC const u32* GetSmoothGroups()
    {
        return m_SmoothGroups;
    }
    IC const Fvector* GetVertices()
    {
        return m_Vertices;
    }
    IC u32 GetVCount()
    ...
    IC const VMapVec& GetVMaps()
    {
        return m_VMaps;
    }
    IC const VMRefsVec& GetVMRefs()
    {
        return m_VMRefs;
    }
    IC const SurfFaces& GetSurfFaces()
    {
        return m_SurfFaces;
    }
