...
bool CExportSkeleton::PrepareGeometry(u8 influence)
{
    ...
    if (m_Source->BoneCount()<1){
        ...
        WriteLog("!..There are no bones in the object.");
     	return false;
    }

    if (m_Source->BoneCount()>MAX_BONE){
        ELog.Msg(mtError,"Object cannot handle more than" TO_STRING( MAX_BONE) " bones.");
        WriteLog("!..Object cannot handle more than" TO_STRING(MAX_BONE) " bones.");
     	return false;
    }
...
{
    u8 SmoothType = 0;
    WriteLog("..Start detecting smooth type");
    bool bRes = true;
    bool Normals = false;
...
//		if (motion->m_Flags.is(esmStopAtEnd)) Msg("%s - %d",motion->Name(),motion->m_Flags.is(esmStopAtEnd));
        ...
        for (int itm_idx=0; itm_idx<b_lst.size(); ++itm_idx)
        {
            ...
            // save
            F.w_u8    (u8((t_present?flTKeyPresent:0)|(r_present?0:flRKeyAbsent)|(bTransform16Bit?flTKey16IsBit:0)|(g_forceFloatTransformQuant ? flTKeyFFT_Bit : 0)));
            if (r_present)
            {
                if (g_forceFloatTransformQuant)
                {
