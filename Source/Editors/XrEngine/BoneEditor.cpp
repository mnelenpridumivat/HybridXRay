...
void SJointIKData::Export(IWriter& F)
{
    ...
    for (int k = 0; k < 3; k++) 
    {
        ...
        if (_min(-limits[k].limit.x, -limits[k].limit.y) != -limits[k].limit.y)
            WriteLog("!..Warning: _min(-limits[k].limit.x, -limits[k].limit.y) != -limits[k].limit.y");

        if (_max(-limits[k].limit.x, -limits[k].limit.y) != -limits[k].limit.x)
            WriteLog("!..Warning: _max(-limits[k].limit.x,-limits[k].limit.y) != -limits[k].limit.x");
