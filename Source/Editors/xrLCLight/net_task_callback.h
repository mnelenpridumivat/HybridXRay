...
public:
    net_task_callback(IAgent* agent, DWORD session): _agent(*agent), _session(session), _beak_count(_break_connection_times) {}

public:
    IC bool break_all() const
    {
        return _beak_count == 0;
    }
    bool          test_connection();
    const IAgent& agent() const
    {
        return _agent;
    }
    IAgent& agent()
