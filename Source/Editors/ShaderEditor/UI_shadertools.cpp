...
{
    // create props
    m_Items = xr_new<UIItemListForm>(); /*TItemList::CreateForm		("Items",				fraLeftBar->paItemList,
                              alClient,TItemList::ilEditMenu|TItemList::ilDragAllowed|TItemList::ilFolderStore);*/
    ...
    m_ItemProps    = xr_new<UIPropertiesForm>();   // TProperties::CreateForm	("Item Properties",
                                                   // fraLeftBar->paShaderProps,	alClient);
    m_PreviewProps = xr_new<UIPropertiesForm>();   //  	= TProperties::CreateForm	("Preview Properties",
                                                   //  fraLeftBar->paPreviewProps,	alClient);
...
void CShaderTool::RegisterTools()
{
    for (int k = aeFirstTool; k < aeMaxTools; k++)
    {
        ...
        {
            case aeEngine:
                tools = xr_new<CSHEngineTools>(ISHInit(EToolsID(k), m_Items, m_ItemProps, m_PreviewProps));
                break;
            case aeCompiler:
                tools = xr_new<CSHCompilerTools>(ISHInit(EToolsID(k), m_Items, m_ItemProps, m_PreviewProps));
                break;
            case aeMtl:
                tools = xr_new<CSHGameMtlTools>(ISHInit(EToolsID(k), m_Items, m_ItemProps, m_PreviewProps));
                break;
            case aeMtlPair:
                tools = xr_new<CSHGameMtlPairTools>(ISHInit(EToolsID(k), m_Items, m_ItemProps, m_PreviewProps));
                break;
            case aeSoundEnv:
                tools = xr_new<CSHSoundEnvTools>(ISHInit(EToolsID(k), m_Items, m_ItemProps, m_PreviewProps));
                break;
        }
        R_ASSERT(tools);
