...
void calc_ogf(xrMU_Model& mu_model)
{
    ...
    for (xrMU_Model::v_subdivs_it it = mu_model.m_subdivs.begin(); it != mu_model.m_subdivs.end(); it++)
    {
        ...
        try
        {
            ...
            {
                clMsg("* ERROR: MU2OGF, model %s, *faces*", *(mu_model.m_name));
            }
        }
        catch (...)
        {
        ...
        {
            clMsg("* ERROR: MU2OGF, [optimize], model %s", *(mu_model.m_name));
        }
        try
        {
            pOGF->CalcBounds();
        ...
        {
            clMsg("* ERROR: MU2OGF, [bounds], model %s", *(mu_model.m_name));
        }
        try
        {
            pOGF->CalculateTB();
        ...
        {
            clMsg("* ERROR: MU2OGF, [calc_tb], model %s", *(mu_model.m_name));
        }
        try
        {
        ...
        {
            clMsg("* ERROR: MU2OGF, [progressive], model %s", *(mu_model.m_name));
        }
        try
        {
            pOGF->Stripify();
        ...
        {
            clMsg("* ERROR: MU2OGF, [stripify], model %s", *(mu_model.m_name));
        }
