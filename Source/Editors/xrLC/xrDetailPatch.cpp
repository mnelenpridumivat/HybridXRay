...
void CBuild::LightPatches()
{
    ...
    for (u32 i = 0; i < g_pathes.size(); i++)
    {
        ...
        if (0 == i % 32)
            Progress(float(i) / float(g_pathes.size()));
    }
    ...
    while (!g_pathes.empty())
    {
        ...
        {
            if (g_pathes[i].dwMaterial != one_subd.front().dwMaterial)
                continue;

            bbtest.set(bb);
            bbtest.modify(g_pathes[i].P);
            bbtest.getsize(size);
            if (size.x > g_merge_limit || size.y > g_merge_limit || size.z > g_merge_limit)
                continue;
