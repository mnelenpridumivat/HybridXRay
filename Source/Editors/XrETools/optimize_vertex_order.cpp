...
const CacheTypeInfo c_ctiCacheTypeSize[c_iCacheNumTypes] = {{12, 1.0f}};
...
const CacheTypeInfo c_ctiCacheTypeSize[c_iCacheNumTypes] = {{20, 0.95f}, {3, 0.05f}};
...
float FindBestScoreLookahead(int iCountdown, float fCurrentScore, float fInputBestSoFar, int iNumTris, WORD* pwResult)
{
    ...
    while (pstCur != NULL)
    {
        ...
        if (iLookahead > 1)
        {
            ...
            // fScore = FindBestScoreLookahead ( iLookahead - 1, fScore, fBestSoFar, pwIndices + 3, iNumTris - 1,
            // pwResult + 3 );
    ...
    else
    {
        ...
        if (iCountdown > 1)
        {
            ...
            // fNewBestScore = FindBestScoreLookahead ( iCountdown - 1, fScore, 1e10, pwIndices + 3, iNumTris - 1,
            // pwResult + 3 );
...
void _OptimiseVertexCoherencyTriList(WORD* pwList, int iHowManyTris, u32 optimize_mode)
{
	...
	float fTemp = FindBestScore (	iHowManyTris - 1,
		...
		pwNewIndex + 3 );
#else
    float fTemp = FindBestScore(iHowManyTris, 0.0f, 1e10, (pstTri.item(0)), iHowManyTris, pwNewIndex);
#endif
