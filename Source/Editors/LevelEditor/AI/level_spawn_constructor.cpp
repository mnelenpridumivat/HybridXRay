...
{
    /*/GRAPH_POINT_STORAGE::iterator	I = m_graph_points.begin();	GRAPH_POINT_STORAGE::iterator	E =
    m_graph_points.end(); for ( ; I != E; ++I) F_entity_Destroy			((ISE_Abstract*&)(*I));
        */
...
{
    m_game_spawn_constructor->add_story_object(
        dynamic_object->CastALifeObject()->m_story_id, dynamic_object, *m_level.name());
}
...
// void CLevelSpawnConstructor::add_group_object					(ISE_Abstract			*abstract, shared_str
// group_section)
//{
//	string256					temp;
    ...
    for (int i = 0; i < (int)m_spawns.size(); i++)
    {
            ...
            !level_graph().inside(level_graph().vertex(m_spawns[i]->m_tNodeID), position))
        {
            ...
            clMsg(
                ...
                *m_spawns[i]->CastAbstract()->s_name, m_spawns[i]->CastAbstract()->name_replace(), VPUSH(position),
                VPUSH(new_position));
        ...
        if (dwBest == u32(-1))
        {
            ...
            S += xr_sprintf(
                ...
                m_spawns[i]->CastAbstract()->o_Position.x, m_spawns[i]->CastAbstract()->o_Position.y,
                m_spawns[i]->CastAbstract()->o_Position.z);
            Msg(S1);
            return false;
        }
...
void CLevelSpawnConstructor::generate_artefact_spawn_positions()
{
    ...
    for (size_t i = 0; i < CountThread; i++)
    {
        ...
        Threads.push_back(
            thread_spawn(&CLevelSpawnConstructor::ThreadGenerateArtefactSpawnPositionStartup, Name, 0, this));
    }
    ...
    //		if (!level_graph().valid_vertex_position(Abstract->o_Position) || !level_graph().inside((*I)->m_tNodeID,
    //Abstract->o_Position))
    ...
    //				Msg						("! CANNOT GENERATE ARTEFACT SPAWN POSITIONS FOR ZONE [%s] ON LEVEL
    //[%s]",zone->name_replace(),*level().name()); 				Msg						("! ZONE [%s] ON LEVEL [%s] IS REMOVED BY AI
    //COMPILER",zone->name_replace(),*level().name());
    //				R_ASSERT3				(zone->m_story_id == INVALID_STORY_ID,"Cannot remove story
    //object",zone->name_replace()); 				R_ASSERT3				(!zone->m_spawn_control,"Cannot remove spawn control
    //object",zone->name_replace()); 				zones.push_back			(zone); 				l_tpaStack.clear		(); 				continue;
    //			}
    // #endif
    //		}
    //		else		*/
    ...
    ////			Msg							("    [%f][%f][%f] :
    ///%f",VPUSH((*I).tPoint),zone->o_Position.distance_to((*I).tPoint));
    //		}
    //
    //		l_tpaStack.clear				();
    //	}
...
void CLevelSpawnConstructor::generate_artefact_spawn_positions_worker()
{
    ...
    while (InterlockedCompareExchange(&m_generate_artefact_spawn_positions_worker_counter, 0, 0))
    {
        ...
        if (!level_graph().valid_vertex_position(Abstract->o_Position) ||
            !level_graph().inside(Object->m_tNodeID, Abstract->o_Position))
        ...
        graph_engine().search(
            level_graph(), Object->m_tNodeID, Object->m_tNodeID, &l_tpaStack,
    ...
    for (u32 i = 0, n = (u32)level_changers().size(); i < n; ++i)
    {
            ...
            if (!xr_strcmp(*level_changers()[i]->m_caLevelPointToChange, (*I)->CastAbstract()->name_replace()))
            {
                ...
                for (u32 ii = 0, nn = game_graph().header().vertex_count(); ii < nn; ++ii)
                {
                    if ((game_graph().vertex(ii)->level_id() != m_level.id()) ||
                        !game_graph().vertex(ii)->level_point().similar((*I)->CastAbstract()->o_Position, .001f))
                        continue;
        ...
        if (!found)
        {
            clMsg(
                ...
                level_changers()[i]->CastAbstract()->name_replace());
            VERIFY(false);
        }
    }
}
...
void CLevelSpawnConstructor::update_artefact_spawn_positions()
{
    ...
    for (; I != E; ++I)
    {
        ...
        if (zone)
        {
            ...
            //			Msg							("%s  %f [%f][%f][%f] : artefact spawn
            //positions",zone->name_replace(),zone->m_fRadius,VPUSH(zone->o_Position)); 			for (u32
            //i=zone->m_artefact_position_offset; i<level_point_count; ++i) 				Msg						(" [%f][%f][%f] :
