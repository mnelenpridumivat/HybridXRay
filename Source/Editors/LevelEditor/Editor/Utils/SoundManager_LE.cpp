...
bool CLevelSoundManager::MakeEnvGeometry(CMemoryWriter& F, bool bErrMsg)
{
    ...
    {
        if (bErrMsg)
            ELog.Msg(mtError, "& Scene hasn't sound environment geometry.");
        return false;
    }
    ...
    for (ObjectIt it = snd_envs.begin(); it != snd_envs.end(); it++)
    {
        ...
        {
            if ((-1 == inner) && (E->m_EnvInner == *e_it))
                inner = e_it - env_names.begin();
            if ((-1 == outer) && (E->m_EnvOuter == *e_it))
                outer = e_it - env_names.begin();
            if ((inner > -1) && (outer > -1))
                break;
        }
        ...
        if (-1 == inner)
        {
            inner = env_names.size();
            env_names.push_back(E->m_EnvInner);
        }
        if (-1 == outer)
        {
