...
void Gizmo::Render()
{
    if (!m_bVisible || Scene->IsPlayInEditor())
        return;
    RCache.set_Z(false);
    if (m_Type == EType::Move)
    {
        ...
        Fvector X;
        X.set(1, 0, 0);
        X.add(m_ScreenPosition);
        Fvector Y;
        Y.set(0, 1, 0);
        Y.add(m_ScreenPosition);
        Fvector Z;
        Z.set(0, 0, 1);
        Z.add(m_ScreenPosition);
        ...
        {
            DU_impl.DrawCone(
                Fidentity, X, Fvector().set(-1, 0, 0), 0.1f, 0.03f,
                color_rgba(m_CurrentStatus == EStatus::SelectedX ? 255 : 127, 0, 0, 255), color_rgba(255, 0, 0, 255),
                TRUE, FALSE);
        }
        DU_impl.DrawCone(
            Fidentity, Y, Fvector().set(0, -1, 0), 0.1f, 0.03f,
            color_rgba(0, m_CurrentStatus == EStatus::SelectedY ? 255 : 127, 0, 255), color_rgba(0, 255, 0, 255), TRUE,
            FALSE);
        if (OBJCLASS_AIMAP != LTools->CurrentClassID())
        {
            DU_impl.DrawCone(
                Fidentity, Z, Fvector().set(0, 0, -1), 0.1f, 0.03f,
                color_rgba(0, 0, m_CurrentStatus == EStatus::SelectedZ ? 255 : 127, 255), color_rgba(0, 0, 255, 255),
                TRUE, FALSE);
        }
    }
    else if (m_Type == EType::Scale)
    {
        ...
        DU_impl.DrawBox(
            m_ScreenPosition, Fvector().set(0.06f, 0.06f, 0.06f), TRUE, FALSE,
            m_CurrentStatus == EStatus::SelectedXYZ ? color_rgba(200, 200, 200, 255) : color_rgba(127, 127, 127, 255),
            color_rgba(255, 0, 0, 255));

        DU_impl.DrawBox(
            X, Fvector().set(0.03f, 0.03f, 0.03f), TRUE, FALSE,
            color_rgba(
                (m_CurrentStatus == EStatus::SelectedX || m_CurrentStatus == EStatus::SelectedXYZ) ? 255 : 127, 0, 0,
                255),
            color_rgba(255, 0, 0, 255));
        DU_impl.DrawBox(
            Y, Fvector().set(0.03f, 0.03f, 0.03f), TRUE, FALSE,
            color_rgba(
                0, (m_CurrentStatus == EStatus::SelectedY || m_CurrentStatus == EStatus::SelectedXYZ) ? 255 : 127, 0,
                255),
            color_rgba(0, 255, 0, 255));
        DU_impl.DrawBox(
            Z, Fvector().set(0.03f, 0.03f, 0.03f), TRUE, FALSE,
            color_rgba(
                0, 0, (m_CurrentStatus == EStatus::SelectedZ || m_CurrentStatus == EStatus::SelectedXYZ) ? 255 : 127,
                255),
    ...
    else if (m_Type == EType::Rotate)
    {
        ...
        DU_impl.DrawCylinder(
            Fidentity, m_ScreenPosition, X, 0.001f, 1.f,
            color_rgba(m_CurrentStatus == EStatus::SelectedX ? 255 : 127, 0, 0, 255),
            color_rgba(m_CurrentStatus == EStatus::SelectedX ? 255 : 127, 0, 0, 255), FALSE, TRUE);
        if (OBJCLASS_AIMAP != LTools->CurrentClassID())
        {
            DU_impl.DrawCylinder(
                Fidentity, m_ScreenPosition, Y, 0.001f, 1.f,
                color_rgba(0, m_CurrentStatus == EStatus::SelectedY ? 255 : 127, 0, 255),
                color_rgba(0, m_CurrentStatus == EStatus::SelectedY ? 255 : 127, 0, 255), FALSE, TRUE);
        }
        DU_impl.DrawCylinder(
            Fidentity, m_ScreenPosition, Z, 0.001f, 1.f,
            color_rgba(0, 0, m_CurrentStatus == EStatus::SelectedZ ? 255 : 127, 255),
            color_rgba(0, 0, m_CurrentStatus == EStatus::SelectedZ ? 255 : 127, 255), FALSE, TRUE);
    }
