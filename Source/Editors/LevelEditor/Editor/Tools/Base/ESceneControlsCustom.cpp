...
bool TUI_CustomControl::HiddenMode()
{
    ...
    {
        case etaSelect:
            return LTools->GetGimzo()->IsFixed() &&
                (LTools->GetGimzo()->GetType() == Gizmo::EType::Rotate && IsSupportRotate());
        case etaAdd:
            return false;
    }
    return false;
    ...
    if (LTools->GetGimzo()->IsFixed())
    {
        if (LTools->GetGimzo()->GetType() == Gizmo::EType::Move)
        {
            ...
            if ((_Shift & ssLeft) || (_Shift & ssRight))
            {
                ...
                if (LTools->GetGimzo()->GetStatus() == Gizmo::EStatus::SelectedX ||
                    LTools->GetGimzo()->GetStatus() == Gizmo::EStatus::SelectedZ)
                {
                    Fplane  XZPlane;
                    Fvector Position;
                    Fvector StartPosition;
                    XZPlane.build_unit_normal(LTools->GetGimzo()->GetStartPosition(), Fvector().set(0, 1, 0));
                    if (XZPlane.intersectRayPoint(UI->m_StartRStart, UI->m_StartRDir, StartPosition) &&
                        XZPlane.intersectRayPoint(UI->m_CurrentRStart, UI->m_CurrentRDir, Position))
                    {
                        Fvector Delta;
                ...
                else if (LTools->GetGimzo()->GetStatus() == Gizmo::EStatus::SelectedY)
                {
                    ...
                    if (YPlane.intersectRayPoint(UI->m_StartRStart, UI->m_StartRDir, StartPosition) &&
                        YPlane.intersectRayPoint(UI->m_CurrentRStart, UI->m_CurrentRDir, Position))
                    {
                        Fvector Delta;
                        Delta.sub(Position, StartPosition);
                ...
                /*   else
                   {
                       ...
                       if (LTools->GetGimzo()->GetStatus() != Gizmo::EStatus::SelectedY) 									amount.y =
                   0.f;
                   }*/
            }
        ...
        else if (LTools->GetGimzo()->GetType() == Gizmo::EType::Scale)
        {
            if ((_Shift & ssLeft) || (_Shift & ssRight))
            {
                ...
                else if (LTools->GetGimzo()->GetStatus() == Gizmo::EStatus::SelectedXYZ)
                {
                    ...
                    if (XYZPlane.intersectRayPoint(UI->m_StartRStart, UI->m_StartRDir, StartPosition) &&
                        XYZPlane.intersectRayPoint(UI->m_CurrentRStart, UI->m_CurrentRDir, Position))
                    {
        ...
        else if (LTools->GetGimzo()->GetType() == Gizmo::EType::Rotate)
        {
            if (_Shift & ssLeft)
            {
                ...
                if (LTools->GetGimzo()->IsStepEnable(Gizmo::EType::Rotate))
                    CHECK_SNAP(m_fRotateSnapAngle, famount, deg2rad(LTools->GetGimzo()->GetStep(Gizmo::EType::Rotate)));
