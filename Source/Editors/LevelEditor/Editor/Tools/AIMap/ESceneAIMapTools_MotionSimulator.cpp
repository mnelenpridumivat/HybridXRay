...
IC void closestPointOnTriangle(Fvector& result, const Fvector* V, const Fvector& p)
{
    ...
    float   dAB = p.distance_to_sqr(Rab);
    Fvector Rbc;
    closestPointOnLine(Rbc, V[1], V[2], p);
    float   dBC = p.distance_to_sqr(Rbc);
    Fvector Rca;
    closestPointOnLine(Rca, V[2], V[0], p);
...
IC void closestPointOnTriangle(Fvector& result, const cl_tri& T, const Fvector& P)
{
    ...
    float   dAB = P.distance_to_sqr(Rab);
    Fvector Rbc;
    closestPointOnEdge(Rbc, T.p[1], T.p[2], T.e21, T.e21s, P);
    float   dBC = P.distance_to_sqr(Rbc);
    Fvector Rca;
    closestPointOnEdge(Rca, T.p[2], T.p[0], T.e02, T.e02s, P);
    float dCA = P.distance_to_sqr(Rca);
