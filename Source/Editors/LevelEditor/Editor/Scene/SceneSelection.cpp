...
int EScene::FrustumSelect(int flag, ObjClassID classfilter)
{
    CFrustum frustum;
    int      count = 0;
    if (!LUI->SelectionFrustum(frustum, classfilter == OBJCLASS_AIMAP ? static_cast<UIAIMapTool*>(GetTool(OBJCLASS_AIMAP)->pCurControl->parent_tool->pForm)->IsIgnoreSceneObjects() : false))
        return 0;
