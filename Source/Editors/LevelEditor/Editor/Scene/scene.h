...
public:
    ...
    IC u32 ToolCount()
    {
        return m_SceneTools.size();
    }

    IC ESceneCustomOTool* GetOTool(ObjClassID cat)
    {
        return dynamic_cast<ESceneCustomOTool*>(GetTool(cat));
    }

    IC SceneToolsMapPairIt FirstTool()
    {
        return m_SceneTools.begin();
    }
    IC SceneToolsMapPairIt LastTool()
    {
        return m_SceneTools.end();
    }

    IC ObjectList& ListObj(ObjClassID cat)
    {
        VERIFY(GetOTool(cat));
        return GetOTool(cat)->GetObjects();
    }
    IC ObjectIt FirstObj(ObjClassID cat)
    {
        VERIFY(GetOTool(cat));
        return ListObj(cat).begin();
    }
    IC ObjectIt LastObj(ObjClassID cat)
    {
        VERIFY(GetOTool(cat));
        return ListObj(cat).end();
    }
    IC int ObjCount(ObjClassID cat)
    {
