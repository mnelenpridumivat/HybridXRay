...
xr_token             summary_texture_type_tokens[] = {{"Base", SSceneSummary::sttBase}, {"Implicit", SSceneSummary::sttImplicit}, {"DO", SSceneSummary::sttDO}, {"Glow", SSceneSummary::sttGlow}, {"LOD", SSceneSummary::sttLOD}, {0, 0}};
...
void SSceneSummary::STextureInfo::OnHighlightClick(ButtonValue* sender, bool& bDataModified, bool& bSafe)
{
    ...
    {
        case 0:
            Scene->HighlightTexture((LPCSTR)sender->tag, false, info.width, info.height, false);
            break;
        case 1:
            Scene->HighlightTexture((LPCSTR)sender->tag, true, info.width, info.height, false);
            break;
        case 2:
            Scene->HighlightTexture((LPCSTR)sender->tag, true, info.width, info.height, true);
            break;
        case 3:
    ...
    if (file_name.size())
    {
        ...
        //. убрал из-за кол-ва > 4096
                ...
                for (objinf_map_it o_it=objects.begin(); o_it!=objects.end(); o_it++){
                    tmp += xr_string().sprintf("%s%s[%d*%3.2f]",tmp.Length()?";
        ...
        */
        if (info.flags.is_any(STextureParams::flDiffuseDetail | STextureParams::flBumpDetail))
        {
            ...
            {
                V = PHelper().CreateChoose(items, PrepareKey(pref.c_str(), "Detail Texture"), &info.detail_name, smTexture);
                V->Owner()->Enable(FALSE);
...
void SSceneSummary::STextureInfo::Export(IWriter* F, u32& mem_use)
{
    ...
    int tex_mem = info.MemoryUsage(*file_name);
    mem_use += tex_mem;
    tmp.sprintf(mask, *file_name, info.FormatString(), info.width, info.height, info.HasAlpha() ? "present" : "absent", iFloor(tex_mem / 1024), effective_area, _sqrt((pixel_area * info.width * info.height) / effective_area), tmp2.c_str(), *info.detail_name, info.detail_scale, *info.bump_name);
...
u32 SSceneSummary::SelectPMColor(float pm)
{
    ...
    float inv_w = 1.f - w;
    return color_rgba(color_get_R(from) * inv_w + color_get_R(to) * w, color_get_G(from) * inv_w + color_get_G(to) * w, color_get_B(from) * inv_w + color_get_B(to) * w, color_get_A(from) * inv_w + color_get_A(to) * w);
}
