...
xr_token summary_texture_type_tokens[] = {{"Base", SSceneSummary::sttBase}, {"Implicit", SSceneSummary::sttImplicit},
                                          {"DO", SSceneSummary::sttDO},     {"Glow", SSceneSummary::sttGlow},
                                          {"LOD", SSceneSummary::sttLOD},   {0, 0}};
...
void SSceneSummary::STextureInfo::FillProp(PropItemVec& items, LPCSTR main_pref, u32& mem_use)
{
    if (file_name.size())
    {
        ...
        PHelper().CreateCaption(
            items, PrepareKey(pref.c_str(), "Size"),
        ...
        PHelper().CreateCaption(
            items, PrepareKey(pref.c_str(), "Pixel Density"),
...
void SSceneSummary::STextureInfo::Export(IWriter* F, u32& mem_use)
{
    ...
    for (objinf_map_it o_it = objects.begin(); o_it != objects.end(); o_it++)
    {
        tmp2 += xr_string().sprintf(
            "%s%s[%d*%3.2f]", tmp2.size() ? "; " : "", o_it->first.c_str(), o_it->second.ref_count, o_it->second.area);
    }
    int tex_mem = info.MemoryUsage(*file_name);
    mem_use += tex_mem;
    tmp.sprintf(
        mask, *file_name, info.FormatString(), info.width, info.height, info.HasAlpha() ? "present" : "absent",
...
bool SSceneSummary::ExportSummaryInfo(LPCSTR fn)
{
    ...
    if (F)
    {
        ...
        F->w_string("[TEXTURES]");
        F->w_string(
            "texture name=format,width,height,alpha,mem usage (Kb),area,pixel density,objects (name[count*area]),detail name,detail scale,bump name");
...
u32 SSceneSummary::SelectPMColor(float pm)
{
    ...
    float inv_w = 1.f - w;
    return color_rgba(
        color_get_R(from) * inv_w + color_get_R(to) * w, color_get_G(from) * inv_w + color_get_G(to) * w,
