...
public:
    ...
    IC bool RefCompare(LPCSTR ref)
    {
        return ref && m_pReference ? (strcmp(ref, m_pReference->GetName()) == 0) : false;
    }
    ...
    {
        return m_pReference ? &m_pReference->Meshes() : 0;
    }
    virtual LPCSTR RefName()
    {
        return m_pReference ? m_pReference->GetName() : 0;
    }
    ...
    IC bool IsDynamic()
    {
        return (m_pReference ? m_pReference->IsDynamic() : false);
    }
    IC bool IsStatic()
    {
        return (m_pReference ? m_pReference->IsStatic() : false);
    }
    IC bool IsMUStatic()
    {
