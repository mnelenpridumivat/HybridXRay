...
{
    Fvector R;
    R.set(-GetRotation().x, -GetRotation().y, -GetRotation().z);
...
void CCustomObject::AnimationFillProp(LPCSTR pref, PropItemVec& items)
{
    ...
    if (Motionable())
    {
        ...
        PHelper().CreateCaption(items, "Motion\\Length (sec)[Old]", FloatTimeToStrTime(m_Motion->GetLength(), true, true, true, true).c_str());
