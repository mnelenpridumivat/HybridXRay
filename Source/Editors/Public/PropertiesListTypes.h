...
public:
    PropValue():
        tag(0), m_Owner(0), OnChangeEvent(0)
    {
        ;
    }
    virtual ~PropValue() {}
...
{
    string32 fmt;
    xr_sprintf(fmt, "%%.%df", dec);
    string256 tmp;
    xr_sprintf(tmp, fmt, V);
    s = tmp;
    return s;
}
//------------------------------------------------------------------------------
...
{
public:
    VectorValue(Fvector* val, float mn, float mx, float increment, int decimal):
...
public:
    xr_token* token;
    TokenValueCustom(xr_token* _token):
        token(_token)
    {
        ;
    }
