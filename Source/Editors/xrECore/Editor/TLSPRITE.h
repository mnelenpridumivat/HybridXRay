...
public:
    ...
    IC void setpos(float x1, float y1, float x2, float y2)
    {
        lb.p.x = x1;
        lb.p.y = y2;
        lt.p.x = x1;
    ...
    IC void setdepth(float z = 0.f, float rhw = 1.f)
    {
        lb.p.z = z;
        rb.p.z = z;
        lt.p.z = z;
    ...
    IC void settex(float x1, float y1, float x2, float y2)
    {
        lb.uv.x = x1;
        lb.uv.y = y2;
        lt.uv.x = x1;
    ...
    IC void to_real(int w, int h)
    {
        ...
        lt.p.y = rt.p.y = TO_REAL(lt.p.y, h);
        lb.p.y = rb.p.y = TO_REAL(lb.p.y, h);
    }
    ...
    IC void to_real(Frect& r, int w, int h)
    {
        ...
        rb.p.x = rt.p.x = TO_REAL(r.x2, w);
        lb.p.y = rb.p.y = TO_REAL(r.y2, h);
    }
