...
RToken16Value*	CScriptPropertiesListHelper::CreateRToken16	(PropItemVec* items, LPCSTR key, luabind::object object,
LPCSTR name, RTokenVec* token)
{
    return		(PHelper().CreateRToken16(*items,key,wrap_value<shared_str>(object,name),token));
}

RToken32Value*	CScriptPropertiesListHelper::CreateRToken32	(PropItemVec* items, LPCSTR key, luabind::object object,
LPCSTR name, RTokenVec* token)
{
