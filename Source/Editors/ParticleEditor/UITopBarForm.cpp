...
void UITopBarForm::Draw()
{
    ...
    ImGui::Begin("TOOLBAR", NULL, window_flags);
    {
        ...
        if (ImGui::ImageButton(m_tSaveParticles->surface_get(), ImVec2(20, 20), ImVec2(0, 0), ImVec2(1, 1), 0))
        {
            ClickSaveParticles();
        }
        if (ImGui::IsItemHovered())
        ...
        if (ImGui::ImageButton(m_tReloadParticles->surface_get(), ImVec2(20, 20), ImVec2(0, 0), ImVec2(1, 1), 0))
        {
        ...
        {
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
            ImGui::SetTooltip("Reload particles from 'rawdata/particles'"_RU >> u8"Перезагрузить партиклы из 'rawdata/particles'");
        }
        ImGui::SameLine();
        ...
        {
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
            ImGui::SetTooltip("Open file .xr"_RU >> u8"Открыть файл .xr");
        }
        ImGui::SameLine();

        m_tSaveXr->Load();
        if (ImGui::ImageButton(m_tSaveXr->surface_get(), ImVec2(20, 20), ImVec2(0, 0), ImVec2(1, 1), 0))
        {
            ClickSaveXr();
        }
        if (ImGui::IsItemHovered())
        {
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
            ImGui::SetTooltip("Save file .xr"_RU >> u8"Сохранить файл .xr");
        }
        ImGui::SameLine();
        ...
        m_tValidate->Load();
        if (ImGui::ImageButton(m_tValidate->surface_get(), ImVec2(20, 20), ImVec2(0, 0), ImVec2(1, 1), 0))
        {
            ClickValidate();
        }
        if (ImGui::IsItemHovered())
        {
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
            ImGui::SetTooltip("Particle Validation."_RU >> u8"Валидация партиклов.");
        }
    }
    ImGui::SameLine(0, 1);
    ImGui::End();
