...
inline Vec3 operator*(const Mat4& m, const Vec3& v)
{
    ...
    if (w == 0.0)
        return Vec3(m[0] * u, m[1] * u, m[2] * u);
    else
        return Vec3(m[0] * u / w, m[1] * u / w, m[2] * u / w);
}
/*
...
extern Mat4   perspective_matrix(double fovy, double aspect, double zmin = 0.0, double zmax = 0.0);
...
inline double trace(const Mat4& m)
{
