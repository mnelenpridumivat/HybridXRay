...
public:
    ...
    template<class U> TVec4(const TVec4<U>& v)
    {
        *this = v;
    }
...
template<class T> inline TVec4<T> operator+(const TVec4<T>& u, const TVec4<T>& v)
{
...
template<class T> inline TVec4<T> cross(const TVec4<T>& a, const TVec4<T>& b, const TVec4<T>& c)
{
    ...
    return TVec4<T>(-a[1] * d1 + a[2] * d2 - a[3] * d3, a[0] * d1 - a[2] * d4 + a[3] * d5, -a[0] * d2 + a[1] * d4 - a[3] * d6, a[0] * d3 - a[1] * d5 + a[2] * d6);
}

template<class T> inline T norm2(const TVec4<T>& v)
{
    return v * v;
}
template<class T> inline T norm(const TVec4<T>& v)
{
