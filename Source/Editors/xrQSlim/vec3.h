...
public:
    ...
    template<class U> TVec3(const TVec3<U>& v)
    {
        *this = v;
    }
...
template<class T> inline TVec3<T> operator+(const TVec3<T>& u, const TVec3<T>& v)
{
...
template<class T> inline T norm2(const TVec3<T>& v)
{
    return v * v;
}
template<class T> inline T norm(const TVec3<T>& v)
{
