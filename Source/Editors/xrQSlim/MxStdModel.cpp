...
{
    for (unsigned int i = 0; i < (unsigned int)face_links.length(); i++)
        xr_delete(face_links[i]);
}
...
void MxStdModel::collect_unmarked_corners(const MxFaceList& faces, MxVertexList& verts)
{
    for (unsigned int i = 0; i < (unsigned int)faces.length(); i++)
        for (unsigned int j = 0; j < 3; j++)
        {
...
void MxStdModel::collect_edge_neighbors(unsigned int v1, unsigned int v2, MxFaceList& faces)
{
...
void MxStdModel::collect_neighborhood(MxVertexID v, int depth, MxFaceList& faces)
{
...
unsigned int MxStdModel::split_edge(unsigned int a, unsigned int b)
{
    ...
    return split_edge(a, b, (v1[0] + v2[0]) / 2.0f, (v1[1] + v2[1]) / 2.0f, (v1[2] + v2[2]) / 2.0f);
}
...
{
    VERIFY(v1 < vert_count());
    VERIFY(v2 < vert_count());
    VERIFY(vertex_is_valid(v1));
    VERIFY(vertex_is_valid(v2));
    VERIFY(v1 != v2);
...
void MxStdModel::contract(MxVertexID v1, MxVertexID v2, const float* vnew, MxPairContraction* conx)
{
...
void MxStdModel::contract(MxVertexID v1, MxVertexID v2, MxVertexID v3, const float* vnew, MxFaceList& changed)
{
...
void MxStdModel::contract(MxVertexID v1, const MxVertexList& rest, const float* vnew, MxFaceList& changed)
{
    unsigned int i;
