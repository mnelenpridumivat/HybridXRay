...
public:
    ...
    inline bool is_in_heap()
    {
        return token != -47;
    }
    inline void not_in_heap()
    {
        token = -47;
    }
    inline int get_heap_pos()
    {
        return token;
    }
    inline void set_heap_pos(int t)
    {
        token = t;
    }

    inline void heap_key(float k)
    {
        import = k;
    }
    inline float heap_key() const
    {
        return import;
    }
};
...
public:
    ...
    MxHeapable* item(unsigned int i)
    {
        return (*this)[i];
    }
    const MxHeapable* item(unsigned int i) const
    {
        return (*this)[i];
    }
    MxHeapable* extract();
    MxHeapable* top()
    {
        return (length() < 1 ? (MxHeapable*)0 : item(0));
    }
