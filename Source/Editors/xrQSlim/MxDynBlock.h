...
template<class T> class MxDynBlock: public MxBlock<T>
{
private:
    int fill;

public:
    MxDynBlock(int n = 2): MxBlock<T>(n)
    {
        fill = 0;
    }
    ...
    typename MxBlock<T>::iterator end()
    {
        return begin() + size();
    }
    typename MxBlock<T>::const_iterator end() const
    {
        return begin() + size();
    }
...
template<class T, int T_SIZE> class MxSizedDynBlock: public MxDynBlock<T>
{
public:
    MxSizedDynBlock(unsigned int n = T_SIZE): MxDynBlock<T>(n) {}
};

template<class T> inline bool varray_find(const MxDynBlock<T>& A, const T& t, unsigned int* index = NULL)
{
