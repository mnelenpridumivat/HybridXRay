...
BOOL  NodeSimilar(vertex& N1, vertex& N2)
{
    ...
    if (!fsimilar(N1.LightLevel, N2.LightLevel, l_eps))
        return FALSE;

    // plane
    if (N1.Plane.n.dotproduct(N2.Plane.n) < _cos(deg2rad(sim_angle)))
        return FALSE;
    if (N1.Plane.distance(N2.Pos) > sim_dist)
        return FALSE;
    if (N2.Plane.distance(N1.Pos) > sim_dist)
        return FALSE;

    // cover
    float c_eps = sim_cover / 256.f;
    if (!fsimilar(N1.cover[0], N2.cover[0], c_eps))
        return FALSE;
    if (!fsimilar(N1.cover[1], N2.cover[1], c_eps))
        return FALSE;
    if (!fsimilar(N1.cover[2], N2.cover[2], c_eps))
        return FALSE;
    if (!fsimilar(N1.cover[3], N2.cover[3], c_eps))
        return FALSE;
...
void             ProcessOne(u32 Base, u32 limit = 8)
{
    ...
    for (;;)
    {
        ...
        if (used[LP])
            break;
        if (!NodeSimilar(BaseNode, g_nodes[LP]))
            break;
    ...
    for (;;)
    {
        ...
        if (used[RP])
            break;
        if (!NodeSimilar(BaseNode, g_nodes[RP]))
            break;
    ...
    for (u32 left = 0; left <= BL_Left; left++)
    {
        ...
        if (limit_right >= BaseLine.size())
            limit_right = BaseLine.size() - 1;
        u32 limit_left = left;
        if (limit_left < BL_Left)
            limit_left = BL_Left;
        ...
        for (int right = int(limit_right); right >= int(limit_left); right--)
        {
            ...
            for (; stack_up.size() <= limit;)
            {
                ...
                for (; I != E; I++)
                {
                    ...
                    if (!NodeSimilar(g_nodes[id], BaseNode))
                    {
                        bFailed = TRUE;
                        break;
                    }
