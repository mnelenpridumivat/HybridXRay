...
{
public:
    MxMatrix(unsigned int n): MxBlock2<double>(n, n)
    {
        *this = 0.0;
    }
    MxMatrix(const MxMatrix& m): MxBlock2<double>(m.dim(), m.dim())
    {
        copy(m);
    }
    ...
    double invert(MxMatrix& inv) const
    {
        return mxm_invert(inv, *this, dim());
    }
};
