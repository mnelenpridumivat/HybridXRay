...
//
template<class T, unsigned int N> class MxVBlock
{
private:
    T elt[N];
...
public:
    ...
    inline T operator*(const MxVBlock<T, N>& v) const
    {
        return mxv_dot(elt, v, N);
    }
...
{
public:
    MxVector(unsigned int n): MxBlock<double>(n)
    {
        mxv_set(*this, 0.0, dim());
    }
    MxVector(const MxVector& v): MxBlock<double>(v.length())
    {
        copy(v);
    }
