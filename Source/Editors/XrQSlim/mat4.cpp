...
{
    return Mat4(Vec4(s[0], 0, 0, 0), Vec4(0, s[1], 0, 0), Vec4(0, 0, s[2], 0), Vec4(0, 0, 0, 1));
}

Mat4 rotation_matrix_rad(double theta, const Vec3& axis)
{
    double c = _cos(theta), s = _sin(theta), xx = axis[0] * axis[0], yy = axis[1] * axis[1], zz = axis[2] * axis[2], xy = axis[0] * axis[1], yz = axis[1] * axis[2], xz = axis[0] * axis[2];
...
{
    return scaling_matrix(Vec3(w / 2.0, -h / 2.0, 1)) * translation_matrix(Vec3(1, -1, 0));
}
