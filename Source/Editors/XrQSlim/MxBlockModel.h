...
public:
    ...
    {
        return (normals ? normals->length() : 0);
    }
    unsigned int texcoord_count() const
    {
        return (tcoords ? tcoords->length() : 0);
    }
    ...
    MxVertexID   add_vertex(float* v)
    {
        return add_vertex(v[0], v[1], v[2]);
    }
    MxFaceID add_face(unsigned int* f)
    {
        return add_face(f[0], f[1], f[2]);
    }
    ...
    MxVertex& corner(MxFaceID f, short i)
    {
        return vertex(face(f)[i]);
    }
    ...
    int color_binding()
    {
        return (cbinding & (binding_mask >> 2));
    }
    int normal_binding()
    {
        return (nbinding & binding_mask);
    }
    int texcoord_binding()
    {
        return (tbinding & (binding_mask >> 4));
    }
