...
public:
    ...
    ~CFileWriteBlock();
    virtual bool save_to(LPCSTR fn)
    {
        return false;
    };
...
public:
    INetMemoryBuffWriter(IGenericStream* _stream, u32 _block_size, u8* buffer): mem_writter(buffer, _block_size), stream(_stream), net_block_write_data_size(_block_size)
    {
        // VERIFY(stream);
    }
