...
template <u32 range> struct fixed16
{
    ...
    void _w(float a)
    {
        s32 _v = iFloor(a * 32767.f / float(range));
        clamp(_v, -32768, 32767);
        _value = _v;
    }
    float _r() const
    {
        return float((_value / 32767.f) * float(range));
    }
};
...
public:
    ...
    {
        mul(1.f / float(samples));
    };
    void max(base_color_c& s)
    {
        rgb.max(s.rgb);
        hemi = _max(hemi, s.hemi);
        sun  = _max(sun, s.sun);
    };
    void lerp(base_color_c& A, base_color_c& B, float s)
    {
