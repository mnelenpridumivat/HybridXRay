    ...
    bool intersect(const Fobb& box, const Fvector& origin, const Fvector& direction, float& dist, Fvector& norm)
    {
        ...
        if (intersect(kOrigin, kDirection, box.m_halfsize, fT0, fT1, ax0, ax1))
        {
            ...
            if (bPick)
            {
                ...
                {
                    case a_px:
                        norm.set(box.m_rotate.i);
                        break;
                    case a_nx:
                        norm.invert(box.m_rotate.i);
                        break;
                    case a_py:
                        norm.set(box.m_rotate.j);
                        break;
                    case a_ny:
                        norm.invert(box.m_rotate.j);
                        break;
                    case a_pz:
                        norm.set(box.m_rotate.k);
                        break;
                    case a_nz:
                        norm.invert(box.m_rotate.k);
                        break;
                    case a_none:
                        NODEFAULT;
                }
    ...
    bool bone_intersect(u16 bone, const IKinematics& K, const Fvector& origin, const Fvector& direction, float& dist, Fvector& norm)
    {
        ...
        switch (bone_shape.type)
        {
            ...
            case SBoneShape::stBox:
                result = intersect(bone_shape.box, l_origin, l_direction, dist, norm);
                break;
            case SBoneShape::stSphere:
                result = intersect(bone_shape.sphere, l_origin, l_direction, dist, norm);
                break;
            case SBoneShape::stCylinder:
                result = intersect(bone_shape.cylinder, l_origin, l_direction, dist, norm);
                break;
            default:
                NODEFAULT;
                return false;
