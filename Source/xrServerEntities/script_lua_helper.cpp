...
void CDbgLuaHelper::DrawVariable(lua_State* l, const char* name, bool bOpenTable)
{
    ...
    switch (ntype)
    {
        ...
        case LUA_TTABLE:
            ...
            /*	case LUA_TUSERDATA:{
                        luabind::detail::object_rep* obj = static_cast<luabind::detail::object_rep*>(lua_touserdata(L,
               -1)); luabind::detail::lua_reference& r = obj->get_lua_table(); lua_State * ls = NULL; r.get(ls);
                        DrawTable(ls, name);
                        return;
                    }break;*/
