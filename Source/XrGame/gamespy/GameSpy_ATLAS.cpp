...
u32 CGameSpy_ATLAS::WSLoginProfile(shared_str const& email, shared_str const& nick, shared_str const& password, WSLoginCallback callback, void* userData)
{
    return static_cast<u32>(xrGS_wsLoginProfile(nick.c_str(), email.c_str(), password.c_str(), NULL, callback, userData));
}

SCResult CGameSpy_ATLAS::CreateSession(const GSLoginCertificate* theCertificate, const GSLoginPrivateData* thePrivateData, SCCreateSessionCallback theCallback, gsi_time theTimeoutMs, void* theUserData)
{
    return xrGS_scCreateSession(m_interface, theCertificate, thePrivateData, theCallback, theTimeoutMs, theUserData);
}

SCResult CGameSpy_ATLAS::SetReportIntention(const gsi_u8 theConnectionId[SC_CONNECTION_GUID_SIZE], gsi_bool isAuthoritative, const GSLoginCertificate* theCertificate, const GSLoginPrivateData* thePrivateData, SCSetReportIntentionCallback theCallback, gsi_time theTimeoutMs, void* theUserData)
{
    return xrGS_scSetReportIntention(m_interface, theConnectionId, isAuthoritative, theCertificate, thePrivateData, theCallback, theTimeoutMs, theUserData);
}
...
SCResult CGameSpy_ATLAS::SubmitReport(const SCReportPtr theReport, gsi_bool isAuthoritative, const GSLoginCertificate* theCertificate, const GSLoginPrivateData* thePrivateData, SCSubmitReportCallback theCallback, gsi_time theTimeoutMs, void* theUserData)
{
    return xrGS_scSubmitReport(m_interface, theReport, isAuthoritative, theCertificate, thePrivateData, theCallback, theTimeoutMs, theUserData);
}

SCResult CGameSpy_ATLAS::CreateReport(gsi_u32 theHeaderVersion, gsi_u32 thePlayerCount, gsi_u32 theTeamCount, SCReportPtr* theReportOut)
{
    return xrGS_scCreateReport(m_interface, theHeaderVersion, thePlayerCount, theTeamCount, theReportOut);
}
...
SCResult CGameSpy_ATLAS::ReportSetPlayerData(SCReportPtr theReport, gsi_u32 thePlayerIndex, const gsi_u8 thePlayerConnectionId[SC_CONNECTION_GUID_SIZE], gsi_u32 thePlayerTeamIndex, SCGameResult theResult, gsi_u32 theProfileId, const GSLoginCertificate* theCertificate)
{
    gsi_u8 zero_auth_data[16];
    ZeroMemory(zero_auth_data, sizeof(zero_auth_data));
    return xrGS_scReportSetPlayerData(theReport, thePlayerIndex, thePlayerConnectionId, thePlayerTeamIndex, theResult, theProfileId, theCertificate, zero_auth_data);
}
