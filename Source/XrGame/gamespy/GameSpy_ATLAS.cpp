...
u32 CGameSpy_ATLAS::WSLoginProfile(
    ...
    void*             userData)
{
    return static_cast<u32>(
        xrGS_wsLoginProfile(nick.c_str(), email.c_str(), password.c_str(), NULL, callback, userData));
}

SCResult CGameSpy_ATLAS::CreateSession(
...
SCResult CGameSpy_ATLAS::SetReportIntention(
    ...
    void*                        theUserData)
{
    return xrGS_scSetReportIntention(
        m_interface, theConnectionId, isAuthoritative, theCertificate, thePrivateData, theCallback, theTimeoutMs,
        theUserData);
}
...
SCResult CGameSpy_ATLAS::SubmitReport(
    ...
    void*                     theUserData)
{
    return xrGS_scSubmitReport(
        m_interface, theReport, isAuthoritative, theCertificate, thePrivateData, theCallback, theTimeoutMs,
        theUserData);
}
...
SCResult CGameSpy_ATLAS::ReportSetPlayerData(
    ...
    const GSLoginCertificate* theCertificate)
{
    ...
    return xrGS_scReportSetPlayerData(
        theReport, thePlayerIndex, thePlayerConnectionId, thePlayerTeamIndex, theResult, theProfileId, theCertificate,
        zero_auth_data);
}
