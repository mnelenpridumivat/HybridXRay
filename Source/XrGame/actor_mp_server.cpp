...
#ifdef DEBUG
    Msg("--- Actor %d[%s] STATE_Read, health is: %2.04f", this->ID, this->name_replace(), m_state_holder.state().health);
#endif   // #ifdef DEBUG
}
...
#ifdef DEBUG
    Msg("--- Actor %d[%s] STATE_Write, health is: %2.04f", this->ID, this->name_replace(), m_state_holder.state().health);
#endif   // #ifdef DEBUG
}
