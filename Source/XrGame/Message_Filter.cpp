    ...
    switch (msg_type.msg_type)
    {
        case M_EVENT: {
            switch (msg_type.msg_subtype)
            {
                case GE_DESTROY: {
                    xr_sprintf(
                        tmp_string, "--- CL_EVENT [%7u][%5u]: GE_DESTROY", msg_type.msg_receive_time,
                ...
                case GE_OWNERSHIP_TAKE: {
                    ...
                    xr_sprintf(
                        tmp_string, "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_TAKE    [%d]", msg_type.msg_receive_time,
                ...
                case GE_OWNERSHIP_REJECT: {
                    ...
                    packet.r_u16(tmp_id_what);
                    xr_sprintf(
                        tmp_string, "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_REJECT  [%d]", msg_type.msg_receive_time,
                        msg_type.dest_obj_id, tmp_id_what);
                }
                break;
                default: {
                    xr_sprintf(
                        tmp_string, "--- CL_EVENT [%7u][%5u]: EVENT_ID=[%d]", msg_type.msg_receive_time,
        ...
        case M_GAMEMESSAGE: {
            switch (msg_type.msg_subtype)
            {
                ...
                default: {
                    xr_sprintf(
                        tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ID=[%d]", msg_type.msg_receive_time,
