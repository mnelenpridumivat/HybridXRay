...
template<typename T> IC void CObjectParams<T>::fill(const T* game_object)
{
    ...
    //!ai().level_graph().inside(m_level_vertex_id,game_object->Position())) { 		m_position			=
    //ai().level_graph().vertex_position(m_level_vertex_id); 		m_position.y		= game_object->Position().y; 		return;
    //	}
