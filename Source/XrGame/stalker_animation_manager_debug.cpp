    ...
    const std::pair<LPCSTR, LPCSTR>* blend_id)
{
    ...
    BLEND_ID query = std::make_pair(
        std::make_pair(animation_id, animation_set_id),
...
void CStalkerAnimationManager::add_animation_stats()
{
    ...
    add_animation_stats(
        m_skeleton_animated->LL_MotionDefName_dbg(torso().animation()), torso().blend_id(m_skeleton_animated, blend),
        torso().m_just_started);
    add_animation_stats(
        m_skeleton_animated->LL_MotionDefName_dbg(legs().animation()), legs().blend_id(m_skeleton_animated, blend),
        legs().m_just_started);
}
#endif   // DEBUG
