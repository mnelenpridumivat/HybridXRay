...
*/
        ...
        {
            //!=R_ASSERT2( 0, make_string( "! Can`t fill <%s> in <UpgradeBase::fill_root_container> for root = %s",
            //!id_str(), root->id_str() ) );
            Groups_type::iterator ib = m_depended_groups.begin();
            Groups_type::iterator ie = m_depended_groups.end();
            for (; ib != ie; ++ib)
        ...
        UpgradeStateResult UpgradeBase::can_install(CInventoryItem& item, bool loading)
        {
            ...
            if (item.has_upgrade(m_id))
            {
                ...
                /*if ( loading )
                {
                    FATAL( make_string( "Loading item: Upgrade <%s> (id = %d) is installed already.", id_str(),
                item.object_id() ).c_str() );
                }*/
