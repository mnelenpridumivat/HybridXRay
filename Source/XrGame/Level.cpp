...
void CLevel::cl_Process_Event(u16 dest, u16 type, NET_Packet& P)
{
    ...
    if (type != GE_DESTROY_REJECT)
    {
        if (type == GE_DESTROY)
        {
            ...
            //			{
            // = GameObject.cpp (210)
            //				Msg( "! ERROR (Level): GE_DESTROY arrived to object[%d][%s], that has parent[%d][%s],
            //frame[%d]", 					GO->ID(), GO->cNameSect().c_str(), 					GO->H_Parent()->ID(), GO->H_Parent()->cName().c_str(),
            //Device->dwFrame );
            //			}
        }
        GO->OnEvent(P, type);
...
void CLevel::OnFrame()
{
    ...
    if (!g_dedicated_server)
    {
        ...
        if (IsGameTypeSingle() && Device->dwPrecacheFrame == 0)
        {
            ...
            //(fastdelegate::FastDelegate0<>(m_game_task_manager,&CGameTaskManager::UpdateTasks)); else
